...
int32_t init_collector_proc_schedstat() {
    __metric_node_schedstat_running_seconds_total =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_schedstat_running_seconds_total",
                             "Number of seconds CPU spent running a process.",
                             1, (const char *[]){ "cpu" }));
    __metric_node_schedstat_waiting_seconds_total =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_schedstat_waiting_seconds_total",
                             "Number of seconds spent by processing waiting "
                             "for this CPU.",
                             1, (const char *[]){ "cpu" }));
    __metric_node_schedstat_timeslices_total =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_schedstat_timeslices_total",
                             "Number of timeslices executed by CPU.", 1,
                             (const char *[]){ "cpu" }));
...
int32_t collector_proc_schedstat(int32_t UNUSED(update_every),
                                 usec_t UNUSED(dt), const char *config_path) {
    debug("[PLUGIN_PROC:proc_schedstat] config:%s running", config_path);

    if (unlikely(!__cfg_proc_schedstat_filename)) {
        __cfg_proc_schedstat_filename = appconfig_get_member_str(
            config_path, "monitor_file", __def_proc_schedstat_filename);
    }

    if (unlikely(!__pf_schedstat)) {
        __pf_schedstat = procfile_open(__cfg_proc_schedstat_filename,
                                       " \t:", PROCFILE_FLAG_DEFAULT);
        if (unlikely(!__pf_schedstat)) {
            error("[PLUGIN_PROC:proc_net_proc_schedstatsockstat] Cannot open "
                  "%s",
    ...
    for (size_t l = 0; l < lines - 1; l++) {
        ...
        if (row_key[0] == 'c' && row_key[1] == 'p' && row_key[2] == 'u') {
            ...
            if (unlikely(words < 10)) {
                error("Cannot read /proc/schedstat cpu line. Expected 9 "
                      "params, read %lu.",
                      words);
                continue;
            }
            ...
            // sum of all time spent running by tasks on this processor (in
            // nanoseconds)
            ...
            schedstat_timeslices =
                str2uint64_t(procfile_lineword(__pf_schedstat, l, 9));

            debug("[PLUGIN_PROC:proc_schedstat] cpu:%s current running:%lf "
                  "waiting:%lf "
