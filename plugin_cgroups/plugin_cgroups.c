...
static const char *__plugin_cgroups_config_base_path = "collector_plugin_"
                                                       "cgroups";
static const char *__plugin_cgroups_v1_ss_base_path = "collector_plugin_"
                                                      "cgroups.cg_base_path.v1";
static const char *__plugin_cgroups_v2_ss_base_path = "collector_plugin_"
                                                      "cgroups.cg_base_path.v2";
...
int32_t cgroup_collector_routine_init() {
    ...
    if (likely(CGROUPS_FAILED != cg_type)) {
        ...
        info("[PLUGIN_CGROUPS] CGroup type: [%s]",
             cg_type == CGROUPS_V1 ? "V1" : "V2");

        // 读取配置，初始化__collector_cgroups
        __collector_cgroups.update_every = appconfig_get_member_int(
            __plugin_cgroups_config_base_path, "update_every", 1);

        __collector_cgroups.update_all_cgroups_interval_secs =
            appconfig_get_member_int(__plugin_cgroups_config_base_path,
                                     "update_all_cgroups_interval_secs", 10);

        __collector_cgroups.ctx.max_cgroups_num = appconfig_get_member_int(
            __plugin_cgroups_config_base_path, "max_cgroups_num", 1000);

        debug("[PLUGIN_CGROUPS] cgroup_collector_routine_init: "
              "update_every=%d, "
        ...
        __collector_cgroups.ctx.cs_cpuacct_enable = appconfig_get_member_bool(
            __plugin_cgroups_config_base_path, "cs_cpuacct_enable", true);
        __collector_cgroups.ctx.cs_memory_enable = appconfig_get_member_bool(
            __plugin_cgroups_config_base_path, "cs_memory_enable", true);
        __collector_cgroups.ctx.cs_cpuset_enable = appconfig_get_member_bool(
            __plugin_cgroups_config_base_path, "cs_cpuset_enable", false);
        __collector_cgroups.ctx.cs_blkio_enable = appconfig_get_member_bool(
            __plugin_cgroups_config_base_path, "cs_blkio_enable", false);
        __collector_cgroups.ctx.cs_device_enable = appconfig_get_member_bool(
            __plugin_cgroups_config_base_path, "cs_device_enable", false);
        ...
        if (CGROUPS_V1 == cg_type) {
            __collector_cgroups.ctx.cs_cpuacct_path = appconfig_get_member_str(
                __plugin_cgroups_v1_ss_base_path, "cs_cpuacct_path", NULL);
            __collector_cgroups.ctx.cs_memory_path =
                appconfig_get_member_str(__plugin_cgroups_v1_ss_base_path,
                                         "cs_memory_path",
                                         "/sys/fs/cgroup/memory");
            __collector_cgroups.ctx.cs_cpuset_path =
                appconfig_get_member_str(__plugin_cgroups_v1_ss_base_path,
                                         "cs_cpuset_path",
                                         "/sys/fs/cgroup/cpuset");
            __collector_cgroups.ctx.cs_blkio_path =
                appconfig_get_member_str(__plugin_cgroups_v1_ss_base_path,
                                         "cs_blkio_path",
                                         "/sys/fs/cgroup/blkio");
            ...
            debug("[PLUGIN_CGROUPS] cg_v1 cs_cpuacct_path:'%s', "
                  "cs_memory_path:'%s', "
                  "cs_cpuset_path:'%s', cs_blkio_path:'%s', "
                  "cs_device_path:'%s'",
            ...
            __collector_cgroups.ctx.unified_path =
                appconfig_get_member_str(__plugin_cgroups_v2_ss_base_path,
                                         "unified_path", "/sys/fs/cgroup");
            debug("[PLUGIN_CGROUPS] cg_v2 unified_path:'%s'",
                  __collector_cgroups.ctx.unified_path);
        }

        const char *cgroup_matching_pattern = appconfig_get_member_str(
            __plugin_cgroups_config_base_path, "cgroups_matching", NULL);
        if (unlikely(!cgroup_matching_pattern)) {
            error("[PLUGIN_CGROUPS] cgroups_matching is not configured");
            return -1;
        } else {
            debug("[PLUGIN_CGROUPS] cgroups_matching: %s",
                  cgroup_matching_pattern);
            __collector_cgroups.ctx.cgroups_matching = simple_pattern_create(
                cgroup_matching_pattern, NULL, SIMPLE_PATTERN_EXACT);
        }

        cgroup_matching_pattern =
            appconfig_get_member_str(__plugin_cgroups_config_base_path,
                                     "cgroups_subpaths_matching", NULL);
        if (unlikely(!cgroup_matching_pattern)) {
            error("[PLUGIN_CGROUPS] cgroups_subpaths_matching is not "
                  "configured");
            return -1;
        } else {
            debug("[PLUGIN_CGROUPS] cgroups_subpaths_matching: %s",
                  cgroup_matching_pattern);
            __collector_cgroups.ctx.cgroups_subpaths_matching =
                simple_pattern_create(cgroup_matching_pattern, NULL,
