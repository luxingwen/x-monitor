...
if [ $? -ne 0 ] ;then
    ...
    cgexec -g memory,cpu:/system.slice/x-monitor nohup ./x-monitor.eBPF --config=../../env/config/xm_ebpf_plugin/config.yaml --log_dir=/var/log/x-monitor/ --v=5 &
else
    ...
    cgexec -g memory,cpu:/x-monitor nohup ./x-monitor.eBPF --config=../../env/config/xm_ebpf_plugin/config.yaml --log_dir=/var/log/x-monitor/ --v=5 &
fi