...
2. 编译libconfig
   ...
   ```
   ~/x-monitor/extra/vendor/libconfig/build$ cmake ../ -DBUILD_SHARED_LIBS=OFF
   -- The C compiler identification is GNU 9.3.0
   -- The CXX compiler identification is GNU 9.3.0
   ~/x-monitor/extra/vendor/libconfig/build$ make -j2
   Scanning dependencies of target config
   ```

3. 安装microhttpd。下载https://ftp.gnu.org/gnu/libmicrohttpd/，可以disable https，减少库依赖。

   ```
   ./configure --disable-https --prefix=/usr --enable-static
   ```

4. 安装libprom，这是Prometheus-c-client库
...
5. 编译pcre2
   ...
   ```
   ~/x-monitor/extra/vendor/pcre2$ ./autogen.sh 
   Looking for a version of libtoolize (which can have different names)...
   ~/x-monitor/extra/vendor/pcre2$ ./configure --without-ssl --disable-shared
   configure: WARNING: unrecognized options: --without-ssl
   ~/x-monitor/extra/vendor/pcre2$ make -j2
   rm -f src/pcre2_chartables.c
   ```

6. 编译libcurl，编译静态库，不支持ssl
...
1. clone代码
   ...
   ```
   git clone https://github.com/wubo0067/x-monitor.git --recursive
   ```

2. 编译ebpf代码，编译libbpf源码生成静态库、生成vmlimux.h、编译x-monitor ebpf代码，将libbpf的头文件和静态库分别安装到extra/include/bpf和extra/libs目录下。
