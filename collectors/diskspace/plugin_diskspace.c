...
static void __collector_diskspace_stats(struct mountinfo *mi,
    ...
    prom_gauge_set(__metric_node_filesystem_avail_bytes,
                   (double)node_filesystem_avail_bytes,
                   (const char *[]){ mi->mount_source, mi->filesystem,
                                     mi->mount_point });
    ...
    prom_gauge_set(__metric_node_filesystem_device_error,
...
int32_t diskspace_routine_init() {
    ...
    __collector_diskspace.excluded_mountpoints =
        simple_pattern_create(appconfig_get_str("collector_plugin_diskspace."
                                                "exclude_mountpoints",
