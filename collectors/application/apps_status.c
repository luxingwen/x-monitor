...
#define APP_METRIC_ADDTO_COLLECTOR(name, metric, collector)                    \
    do {                                                                       \
        metric = prom_gauge_new(                                               \
            TO_STRING(APP_METRIC_PREFIX) "_" #name,                            \
            __app_metric_##name##_help, 2,                                     \
    ...
    if (likely(as)) {
        ...
        if (unlikely(
                ...
                != prom_collector_registry_register_collector(
                    PROM_COLLECTOR_REGISTRY_DEFAULT, as->app_prom_collector))) {
        ...
        as->metrics.metric_majflt = prom_gauge_new(
            ...
            (const char *[]){ TO_STRING(APP_METRIC_LABEL_NAME), "app_type" });
        prom_collector_add_metric(as->app_prom_collector,
                                  as->metrics.metric_majflt);

        as->metrics.metric_minflt = prom_gauge_new(
            TO_STRING(APP_METRIC_PREFIX) "_minor_page_faults_total",
            __app_metric_minflt_help, 2,
            (const char *[]){ TO_STRING(APP_METRIC_LABEL_NAME), "app_type" });
        prom_collector_add_metric(as->app_prom_collector,
                                  as->metrics.metric_minflt);
        ...
        APP_METRIC_ADDTO_COLLECTOR(cpu_utime, as->metrics.metric_cpu_utime,
                                   as->app_prom_collector);
        APP_METRIC_ADDTO_COLLECTOR(cpu_stime, as->metrics.metric_cpu_stime,
                                   as->app_prom_collector);
        ...
        APP_METRIC_ADDTO_COLLECTOR(cpu_cstime, as->metrics.metric_cpu_cstime,
                                   as->app_prom_collector);
        APP_METRIC_ADDTO_COLLECTOR(cpu_total_jiffies,
                                   as->metrics.metric_cpu_total_jiffies,
                                   as->app_prom_collector);

        APP_METRIC_ADDTO_COLLECTOR(num_threads, as->metrics.metric_num_threads,
                                   as->app_prom_collector);

        APP_METRIC_ADDTO_COLLECTOR(vmsize_kilobytes,
                                   as->metrics.metric_vmsize_kilobytes,
                                   as->app_prom_collector);
        APP_METRIC_ADDTO_COLLECTOR(vmrss_kilobytes,
                                   as->metrics.metric_vmrss_kilobytes,
                                   as->app_prom_collector);
        ...
        APP_METRIC_ADDTO_COLLECTOR(mem_rss_shmem_kilobytes,
                                   ...
                                   as->app_prom_collector);
        APP_METRIC_ADDTO_COLLECTOR(mem_vmswap_kilobytes,
                                   as->metrics.metric_mem_vmswap_kilobytes,
                                   as->app_prom_collector);
        APP_METRIC_ADDTO_COLLECTOR(mem_pss_kilobytes,
                                   as->metrics.metric_mem_pss_kilobytes,
                                   as->app_prom_collector);
        ...
        APP_METRIC_ADDTO_COLLECTOR(mem_pss_shmem_kilobytes,
                                   ...
                                   as->app_prom_collector);
        APP_METRIC_ADDTO_COLLECTOR(mem_uss_kilobytes,
                                   as->metrics.metric_mem_uss_kilobytes,
                                   as->app_prom_collector);

        APP_METRIC_ADDTO_COLLECTOR(io_logical_bytes_read,
                                   as->metrics.metric_io_logical_bytes_read,
                                   as->app_prom_collector);
        APP_METRIC_ADDTO_COLLECTOR(io_logical_bytes_written,
                                   as->metrics.metric_io_logical_bytes_written,
                                   as->app_prom_collector);
        APP_METRIC_ADDTO_COLLECTOR(io_read_calls,
                                   as->metrics.metric_io_read_calls,
                                   as->app_prom_collector);
        APP_METRIC_ADDTO_COLLECTOR(io_write_calls,
                                   as->metrics.metric_io_write_calls,
                                   as->app_prom_collector);
        APP_METRIC_ADDTO_COLLECTOR(io_storage_bytes_read,
                                   as->metrics.metric_io_storage_bytes_read,
                                   as->app_prom_collector);
        ...
        APP_METRIC_ADDTO_COLLECTOR(open_fds, as->metrics.metric_open_fds,
                                   as->app_prom_collector);
        APP_METRIC_ADDTO_COLLECTOR(max_oom_score,
                                   as->metrics.metric_max_oom_score,
                                   as->app_prom_collector);
        APP_METRIC_ADDTO_COLLECTOR(max_oom_score_adj,
                                   as->metrics.metric_max_oom_score_adj,
                                   as->app_prom_collector);

        as->metrics.metric_nvcsw = prom_gauge_new(
            TO_STRING(APP_METRIC_PREFIX) "_voluntary_ctxt_switches_total",
            __app_metric_nvcsw_help, 2,
            (const char *[]){ TO_STRING(APP_METRIC_LABEL_NAME), "app_type" });
        prom_collector_add_metric(as->app_prom_collector,
                                  as->metrics.metric_nvcsw);

        as->metrics.metric_nivcsw = prom_gauge_new(
            TO_STRING(APP_METRIC_PREFIX) "_novoluntary_ctxt_switches_total",
...
#define APP_SOCK_METRIC_ADDTO_COLLECTOR(metric_name, metric, collector)     \
    do {                                                                    \
        metric = prom_gauge_new(                                            \
            TO_STRING(APP_METRIC_PREFIX) "_" #metric_name,                  \
        ...
        debug("[PLUGIN_APPSTATUS] add app_status for app '%s' with pid: %d",
              app_name, pid);
    }
    return as;
}
...
static void __del_app_assoc_process(struct app_assoc_process *aap,
    ...
    if (likely(aap)) {
        ...
        debug("[PLUGIN_APPSTATUS] del app_assoc_process for pid: %d, app_pid: "
              ...
              aap->as_target->app_name, aap->as_target->process_count);
        aap->as_target = NULL;
        // 释放进程统计对象
...
// 构造应用进程关联结构对象
static struct app_assoc_process *__get_app_assoc_process(struct app_status *as,
                                                         pid_t pid,
                                                         pid_t app_pid,
                                                         const char *app_name) {
    ...
    if (unlikely(cc_hashtable_get(__app_assoc_process_table, &pid, (void *)&aap)
        ...
        if (likely(NULL != aap->as_target && aap->as_target->app_pid == app_pid
            ...
            warn("[PLUGIN_APPSTATUS] app_assoc_process already exists for pid: "
                 "%d, exist_app_pid: %d, exist_app_name: '%s', curr_app_pid: "
                 "%d, curr_app_name: "
                 "'%s', but the app_status is not the same, so delete it",
                 pid, aap->as_target->app_pid, aap->as_target->app_name,
                 app_pid, app_name);
    ...
    // 构造一个新的关联对象
    aap =
        (struct app_assoc_process *)xm_mempool_malloc(__app_assoc_process_xmp);
    if (likely(aap)) {
        memset(aap, 0, sizeof(struct app_assoc_process));
        aap->as_target = as;
        aap->ps_target = new_process_status(pid, __process_status_xmp);
        aap->update = 0;
        cc_hashtable_add(__app_assoc_process_table,
                         (void *)&(aap->ps_target->pid), (void *)aap);
        as->process_count++;
        ...
        if (rule->assign_type == APP_BIND_PROCESSTREE) {
            ...
            if (likely(-1 != get_process_descendant_pids(pid, &pd_pids))) {
                ...
                for (size_t p_i = 0; p_i < pd_pids.pids_size; p_i++) {
                    __get_app_assoc_process(as, pd_pids.pids[p_i], app_pid,
                                            rule->app_name);
                }
                free(pd_pids.pids);
            } else {
...
int32_t init_apps_collector() {
    if (!__app_assoc_process_table) {
        ...
        enum cc_stat status =
            cc_hashtable_new_conf(&config, &__app_assoc_process_table);
    ...
    if (!__process_status_xmp) {
        __process_status_xmp =
            xm_mempool_init(sizeof(struct process_status), 1024, 128);
    }

    if (!__app_assoc_process_xmp) {
        __app_assoc_process_xmp =
            xm_mempool_init(sizeof(struct app_assoc_process), 1024, 128);
    }
    return 0;
}
...
int32_t update_app_collection(struct app_filter_rules *afr) {
    ...
    debug("[PLUGIN_APPSTATUS] start update app collection with %d filter rules",
          afr->rule_count);

    DIR *dir = opendir("/proc");
    if (unlikely(!dir)) {
        error("[PLUGIN_APPSTATUS] opendir /proc failed. error: %s",
              strerror(errno));
        return -1;
    }
...
static void __set_app_sock_metrics(struct app_status *as, const char *app_name,
                                   const char *app_type_name) {
    prom_gauge_set(as->metrics.metric_sock_fds, as->sst.tcp_established,
                   (const char *[]){ app_name, app_type_name,
                                     sock_state_name(SS_ESTABLISHED), "Tcp" });
    prom_gauge_set(as->metrics.metric_sock_fds, as->sst.tcp_close_wait,
                   (const char *[]){ app_name, app_type_name,
                                     sock_state_name(SS_CLOSE_WAIT), "Tcp" });
    prom_gauge_set(as->metrics.metric_sock_fds, as->sst.tcp_listen,
                   (const char *[]){ app_name, app_type_name,
                                     sock_state_name(SS_LISTEN), "Tcp" });

    prom_gauge_set(as->metrics.metric_sock_fds, as->sst.tcp6_established,
                   (const char *[]){ app_name, app_type_name,
                                     sock_state_name(SS_ESTABLISHED), "Tcp6" });
    prom_gauge_set(as->metrics.metric_sock_fds, as->sst.tcp6_close_wait,
                   (const char *[]){ app_name, app_type_name,
                                     sock_state_name(SS_CLOSE_WAIT), "Tcp6" });
    prom_gauge_set(as->metrics.metric_sock_fds, as->sst.tcp6_listen,
                   (const char *[]){ app_name, app_type_name,
                                     sock_state_name(SS_LISTEN), "Tcp6" });

    prom_gauge_set(as->metrics.metric_sock_fds, as->sst.udp_established,
                   (const char *[]){ app_name, app_type_name,
                                     sock_state_name(SS_ESTABLISHED), "Udp" });

    prom_gauge_set(as->metrics.metric_sock_fds, as->sst.udp6_close,
                   (const char *[]){ app_name, app_type_name,
                                     sock_state_name(SS_CLOSE), "Udp" });

    prom_gauge_set(as->metrics.metric_sock_fds, as->sst.udp6_established,
                   (const char *[]){ app_name, app_type_name,
                                     sock_state_name(SS_ESTABLISHED), "Udp6" });
    prom_gauge_set(as->metrics.metric_sock_fds, as->sst.udp_close,
                   (const char *[]){ app_name, app_type_name,
                                     sock_state_name(SS_CLOSE), "Udp6" });

    prom_gauge_set(as->metrics.metric_sock_fds, as->sst.unix_established,
                   (const char *[]){ app_name, app_type_name,
                                     sock_state_name(SS_ESTABLISHED),
                                     "UnixDomain" });
    prom_gauge_set(as->metrics.metric_sock_fds, as->sst.unix_recv,
                   (const char *[]){ app_name, app_type_name,
                                     sock_state_name(SS_SYN_RECV),
                                     "UnixDomain" });
}
...
int32_t collecting_apps_usage(/*struct app_filter_rules *afr*/) {
    ...
    while (cc_hashtable_iter_next(&iter_hash, &next_entry) != CC_ITER_END) {
        ...
        if (unlikely(0 != ret)) {
            ...
            if (likely(
                ...
                as->vmsize += ps->vmsize;
                as->vmrss += ps->smaps.rss;
                as->rss_anon += ps->rss_anon;
                as->rss_file += ps->rss_file;
                as->rss_shmem += ps->rss_shmem;
                as->vmswap += ps->vmswap;
                as->pss += ps->smaps.pss;
                as->pss_anon += ps->smaps.pss_anon;
                as->pss_file += ps->smaps.pss_file;
                as->pss_shmem += ps->smaps.pss_shmem;
                as->uss += ps->smaps.uss;
    ...
    list_for_each_safe (iter_list, n, &__app_status_list) {
        ...
        if (0 == as->process_count) {
            info("[PLUGIN_APPSTATUS] app '%s' app_pid: %d to be cleaned up",
                 as->app_name, as->app_pid);
            // 删除应用指标收集对象
    ...
    __list_for_each(iter_list, &__app_status_list) {
        ...
        prom_gauge_set(as->metrics.metric_cpu_cstime, as->cstime_raw,
                       (const char *[]){ app_name, app_type_name });
        prom_gauge_set(as->metrics.metric_cpu_total_jiffies,
                       as->app_total_jiffies,
                       (const char *[]){ app_name, app_type_name });
        ...
        prom_gauge_set(as->metrics.metric_mem_rss_file_kilobytes, as->rss_file,
                       (const char *[]){ app_name, app_type_name });
        prom_gauge_set(as->metrics.metric_mem_rss_shmem_kilobytes,
                       as->rss_shmem,
                       (const char *[]){ app_name, app_type_name });
        ...
        prom_gauge_set(as->metrics.metric_mem_pss_file_kilobytes, as->pss_file,
                       (const char *[]){ app_name, app_type_name });
        prom_gauge_set(as->metrics.metric_mem_pss_shmem_kilobytes,
                       as->pss_shmem,
                       (const char *[]){ app_name, app_type_name });

        prom_gauge_set(as->metrics.metric_mem_uss_kilobytes, as->uss,
                       (const char *[]){ app_name, app_type_name });
        prom_gauge_set(as->metrics.metric_io_logical_bytes_read,
                       as->io_logical_bytes_read,
                       (const char *[]){ app_name, app_type_name });
        prom_gauge_set(as->metrics.metric_io_logical_bytes_written,
                       as->io_logical_bytes_written,
                       (const char *[]){ app_name, app_type_name });
        ...
        prom_gauge_set(as->metrics.metric_io_write_calls, as->io_write_calls,
                       (const char *[]){ app_name, app_type_name });
        prom_gauge_set(as->metrics.metric_io_storage_bytes_read,
                       as->io_storage_bytes_read,
                       (const char *[]){ app_name, app_type_name });
        prom_gauge_set(as->metrics.metric_io_storage_bytes_written,
                       as->io_storage_bytes_written,
                       (const char *[]){ app_name, app_type_name });
        prom_gauge_set(as->metrics.metric_io_cancelled_write_bytes,
                       as->io_cancelled_write_bytes,
                       (const char *[]){ app_name, app_type_name });
        prom_gauge_set(as->metrics.metric_open_fds, as->open_fds,
                       (const char *[]){ app_name, app_type_name });
        prom_gauge_set(as->metrics.metric_max_oom_score, as->max_oom_score,
                       (const char *[]){ app_name, app_type_name });
        prom_gauge_set(as->metrics.metric_max_oom_score_adj,
                       as->max_oom_score_adj,
                       (const char *[]){ app_name, app_type_name });
        ...
        debug("[PLUGIN_APPSTATUS] app '%s' minflt: %lu, cminflt: %lu, "
              ...
              "cstime: %lu, app_total_jiffies: %lu, app_num_threads: %d, "
              "vmsize: %lu kB, "
              "vmrss: %lu kB, rss_anon: %lu kB, rss_file: %lu kB, rss_shmem: "
              "%lu kB, pss: %lu kB, "
              "pss_anon: %lu kB, pss_file %lu kB, pss_shmem %lu kB, uss: %lu "
              "kB, "
              "io_logical_bytes_read: %lu, io_logical_bytes_written: %lu, "
              "io_read_calls: %lu, "
              "io_write_calls: %lu, io_storage_bytes_read: %lu, "
              "io_storage_bytes_written: %lu, "
              "io_cancelled_write_bytes: %d, open_fds: %d, max_oom_score: %d, "
              "max_oom_score_adj: %d",
              as->app_name, as->minflt_raw, as->cminflt_raw, as->majflt_raw,
              as->cmajflt_raw, as->utime_raw, as->stime_raw, as->cutime_raw,
              as->cstime_raw, as->app_total_jiffies, as->num_threads,
              as->vmsize, as->vmrss, as->rss_anon, as->rss_file, as->rss_shmem,
              as->pss, as->pss_anon, as->pss_file, as->pss_shmem, as->uss,
              as->io_logical_bytes_read, as->io_logical_bytes_written,
              as->io_read_calls, as->io_write_calls, as->io_storage_bytes_read,
              as->io_storage_bytes_written, as->io_cancelled_write_bytes,
