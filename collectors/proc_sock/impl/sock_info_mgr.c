...
__hidden int32_t init_sock_info_mgr() {
    RUN_ONCE({
        ...
        g_proc_sock_info_mgr = (struct proc_sock_info_mgr *)calloc(
            1, sizeof(struct proc_sock_info_mgr));
        ...
        // 构造rcu hash table
        g_proc_sock_info_mgr->sock_info_node_rcu_ht =
            cds_lfht_new_flavor(8, 8, 0,
        ...
        // pthread_spin_init(&g_proc_sock_info_mgr->spin_lock,
...
static void __free_sock_info_node(struct rcu_head *rcu_node) {
    struct sock_info_node *sin =
        container_of(rcu_node, struct sock_info_node, rcu_node);
    debug("[PROC_SOCK] free sock_info_node ino:%u", sin->si.ino);
    xm_mempool_free(g_proc_sock_info_mgr->sock_info_node_xmp, sin);
}

/**
 * It returns 0 if the inode number of the socket info structure pointed to by
 * the node parameter is equal to the inode number pointed to by the key
 * parameter
 *
 ...
 */
static int32_t __match_sock_info(struct cds_lfht_node *ht_node,
                                 const void *key) {
    const uint32_t *p_ino = (const uint32_t *)key;

    struct sock_info_node *sin =
        container_of(ht_node, struct sock_info_node, hash_node);
...
__hidden int32_t add_sock_info_node(struct sock_info_node *new_sin) {
    ...
    if (likely(new_sin)) {
        ...
        cds_lfht_lookup(g_proc_sock_info_mgr->sock_info_node_rcu_ht, hash,
                        __match_sock_info, &new_sin->si.ino, &iter);
        ...
        if (likely(ht_node)) {
            ...
            if (old_sin->si.sock_type != new_sin->si.sock_type
                ...
                xm_mempool_free(g_proc_sock_info_mgr->sock_info_node_xmp,
                                new_sin);
            }
        } else {
            use_new = true;
        ...
        if (use_new) {
            // 替换或者添加
            ht_node = cds_lfht_add_replace(
                g_proc_sock_info_mgr->sock_info_node_rcu_ht, hash,
                __match_sock_info, &new_sin->si.ino, &new_sin->hash_node);
            if (ht_node) {
                debug("[PROC_SOCK] replace old sock_info_node ino:%u, type:%d, "
                      "state:%d ===> new "
                      "sock_info_node ino:%d, type:%d, state:%d",
                      old_sin->si.ino, old_sin->si.sock_type,
                      old_sin->si.sock_state, new_sin->si.ino,
                      new_sin->si.sock_type, new_sin->si.sock_state);

                old_sin =
                    container_of(ht_node, struct sock_info_node, hash_node);
                urcu_memb_call_rcu(&old_sin->rcu_node, __free_sock_info_node);
            } else {
                debug("[PROC_SOCK] add sock_info_node ino:%u, type:%d, "
                      "is_update:%d, hash:%u",
                      new_sin->si.ino, new_sin->si.sock_type,
                      atomic_read(&new_sin->is_update), hash);
            }
        }
...
__hidden uint32_t sock_info_count_i() {
    ...
    urcu_memb_read_lock();
    cds_lfht_for_each_entry(g_proc_sock_info_mgr->sock_info_node_rcu_ht, &iter,
                            sin, hash_node) {
        count++;
    }
    urcu_memb_read_unlock();
...
/**
 * It iterates over all the entries in the hash table, deletes them, and then
 * calls the RCU callback function to free the memory
 */
static void __remove_all_sock_info() {
    ...
    cds_lfht_for_each_entry(g_proc_sock_info_mgr->sock_info_node_rcu_ht, &iter,
        ...
        ht_node = cds_lfht_iter_get_node(&iter);
        ret =
            cds_lfht_del(g_proc_sock_info_mgr->sock_info_node_rcu_ht, ht_node);
        if (!ret) {
            // debug("[PROC_SOCK] remove sock_info_node ino: %u, sock_type:%d",
            // sin->si.ino,
            //       sin->si.sock_type);
...
__hidden void remove_all_not_update_sock_info() {
    ...
    cds_lfht_for_each_entry(g_proc_sock_info_mgr->sock_info_node_rcu_ht, &iter,
        ...
        if (atomic_read(&sin->is_update) == 0) {
            debug("[PROC_SOCK] remove not update sock_info_node ino: %u, "
                  "sock_type:%d",
                  sin->si.ino, sin->si.sock_type);
            ret = cds_lfht_del(g_proc_sock_info_mgr->sock_info_node_rcu_ht,
                               ht_node);
            if (!ret) {
                urcu_memb_call_rcu(&sin->rcu_node, __free_sock_info_node);
            }
