...
#endif
// static const double __kernel_sector_kb = (double)__kernel_sector_size /
// 1024.0;
//
...
static prom_gauge_t *__metric_node_disk_io_now = NULL,
                    *__metric_node_disk_iostat_tps = NULL,
...
static enum disk_type __get_device_type(const char *dev_name, uint32_t major,
    ...
    if (likely(access(buffer, R_OK) == 0)) {
        // assign it here, but it will be overwritten if it is not a physical
        // disk
        dt = DISK_TYPE_PHYSICAL;
    } else {
        snprintf(buffer, XM_DEV_NAME_MAX, "/sys/dev/block/%u:%u/partition",
                 major, minor);
        if (likely(access(buffer, R_OK) == 0)) {
            dt = DISK_TYPE_PARTITION;
        } else {
            snprintf(buffer, XM_DEV_NAME_MAX, "/sys/dev/block/%u:%u/slaves",
                     major, minor);
...
static struct io_device *__get_device(const char *device_name, uint32_t major,
                                      uint32_t minor) {
    ...
    debug("[PLUGIN_PROC:proc_diskstats] %s:%u:%u:%u", device_name, major, minor,
          dev->sector_size);
...
int32_t init_collector_proc_diskstats() {
    __metric_node_disk_reads_completed_total_count =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_disk_reads_completed_total_count",
                             "The total number of reads completed "
                             "successfully.",
                             1, (const char *[]){ "device" }));

    __metric_node_disk_reads_merged_total_count =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_disk_reads_merged_total_count",
                             "The total number of reads merged.", 1,
                             (const char *[]){ "device" }));

    __metric_node_disk_read_total_bytes =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_disk_read_total_bytes",
                             "The total number of bytes read successfully.", 1,
                             (const char *[]){ "device" }));

    __metric_node_disk_read_spent_total_seconds =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_disk_read_spent_total_seconds",
                             "The total number of seconds spent by all reads.",
                             1, (const char *[]){ "device" }));

    __metric_node_disk_writes_completed_total_count =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_disk_writes_completed_total_count",
                             "The total number of writes completed "
                             "successfully.",
                             1, (const char *[]){ "device" }));

    __metric_node_disk_writes_merged_total_count =
        prom_collector_registry_must_register_metric(prom_counter_new(
            "node_disk_writes_merged_total_count",
            "The number of writes merged.", 1, (const char *[]){ "device" }));

    __metric_node_disk_writes_total_bytes =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_disk_writes_total_bytes",
                             "The total number of bytes written successfully.",
                             1, (const char *[]){ "device" }));

    __metric_node_disk_write_spent_total_seconds =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_disk_write_spent_total_seconds",
                             "This is the total number of seconds spent by all "
                             "writes.",
                             1, (const char *[]){ "device" }));

    __metric_node_disk_io_now = prom_collector_registry_must_register_metric(
        prom_gauge_new("node_disk_io_now",
                       "The number of I/Os currently in progress.", 1,
                       (const char *[]){ "device" }));

    __metric_node_disk_io_spent_total_seconds =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_disk_io_spent_total_seconds",
                             "Total seconds spent doing I/Os.", 1,
                             (const char *[]){ "device" }));

    __metric_node_disk_io_time_weighted_total_seconds =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_disk_io_time_weighted_total_seconds",
                             "The weighted # of seconds spent doing I/Os.", 1,
                             (const char *[]){ "device" }));

    __metric_node_disk_discards_completed_total_count =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_disk_discards_completed_total_count",
                             "The total number of discards completed "
                             "successfully.",
                             1, (const char *[]){ "device" }));

    __metric_node_disk_discards_merged_total_count =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_disk_discards_merged_total_count",
                             "The total number of discards merged.", 1,
                             (const char *[]){ "device" }));

    __metric_node_discarded_sectors_total_count =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_discarded_sectors_total_count",
                             "The total number of sectors discarded "
                             "successfully.",
                             1, (const char *[]){ "device" }));

    __metric_node_disk_discard_spent_total_seconds =
        prom_collector_registry_must_register_metric(
    ...
    __metric_node_disk_iostat_tps =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_disk_iostat_tps",
    ...
    __metric_node_disk_iostat_read_requests_per_sec =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_disk_iostat_read_requests_per_sec",
    ...
    __metric_node_disk_iostat_write_requests_per_sec =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_disk_iostat_write_requests_per_sec",
                           "The number (after merges) of write requests "
                           "completed per second for the device.",
                           1, (const char *[]){ "device" }));

    __metric_node_disk_iostat_read_kilobytes_per_sec =
        prom_collector_registry_must_register_metric(
    ...
    __metric_node_disk_iostat_read_requests_merged_per_sec =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_disk_iostat_read_request_merged_per_sec",
    ...
    __metric_node_disk_iostat_write_requests_merged_per_sec =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_disk_iostat_write_request_merged_per_sec",
                           "The number of write requests merged per second "
                           "that were queued to the device.",
                           1, (const char *[]){ "device" }));

    __metric_node_disk_iostat_r_await_msecs =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_disk_iostat_r_await",
                           "The average time (in milliseconds) for read "
                           "requests issued to the device "
                           "to be served. This includes the time spent by the "
                           "requests in queue and "
                           "the time spent servicing them.",
                           1, (const char *[]){ "device" }));

    __metric_node_disk_iostat_w_await_msecs =
        prom_collector_registry_must_register_metric(
    ...
    __metric_node_disk_iostat_await_msecs =
        prom_collector_registry_must_register_metric(
    ...
    __metric_node_disk_iostat_aqu_sz =
        prom_collector_registry_must_register_metric(
    ...
    __metric_node_disk_iostat_areq_sz =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_disk_iostat_avg_request_kilobytes",
                           "The average size (in kilobytes) of the I/O "
                           "requests that were issued to the device.",
                           1, (const char *[]){ "device" }));

    __metric_node_disk_iostat_rareq_sz =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_disk_iostat_avg_request_read_kilobytes",
    ...
    __metric_node_disk_iostat_wareq_sz =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_disk_iostat_avg_request_write_kilobytes",
                           "The average size (in kilobytes) of the write "
                           "requests that were issued to the device.",
                           1, (const char *[]){ "device" }));

    __metric_node_disk_iostat_util =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_disk_iostat_util",
                           "Percentage of elapsed time during which I/O "
...
int32_t collector_proc_diskstats(int32_t UNUSED(update_every), usec_t dt,
    ...
    if (unlikely(!__cfg_proc_diskstat_filename)) {
        __cfg_proc_diskstat_filename = appconfig_get_member_str(
            config_path, "monitor_file", __def_proc_diskstat_filename);
    }

    if (unlikely(!__pf_diskstats)) {
        __pf_diskstats = procfile_open(__cfg_proc_diskstat_filename, " \t",
                                       PROCFILE_FLAG_DEFAULT);
        if (unlikely(!__pf_diskstats)) {
            error("[PLUGIN_PROC:proc_diskstats] Cannot open /proc/diskstats");
            return -1;
        }
        debug("[PLUGIN_PROC:proc_diskstats] opened '%s'",
              procfile_filename(__pf_diskstats));
    }
    ...
    for (l = 0; l < pf_diskstats_lines; l++) {
        ...
        curr_devstats->rd_ios = str2uint64_t(
            procfile_lineword(__pf_diskstats, l, 3)); // prometheus.ReadIOs
        curr_devstats->wr_ios =
            str2uint64_t(procfile_lineword(__pf_diskstats, l, 7)); // WriteIOs

        curr_devstats->rd_merges =
            str2uint64_t(procfile_lineword(__pf_diskstats, l, 4)); // ReadMerges
        curr_devstats->wr_merges = str2uint64_t(
            procfile_lineword(__pf_diskstats, l, 8)); // WriteMerges

        curr_devstats->rd_sectors = str2uint64_t(
            procfile_lineword(__pf_diskstats, l, 5)); // ReadSectors
        curr_devstats->wr_sectors = str2uint64_t(
            procfile_lineword(__pf_diskstats, l, 9)); // WriteSectors

        curr_devstats->rd_ticks =
            str2uint64_t(procfile_lineword(__pf_diskstats, l, 6)); // ReadTicks
        curr_devstats->wr_ticks = str2uint64_t(
            procfile_lineword(__pf_diskstats, l, 10)); // WriteTicks

        curr_devstats->ios_pgr = str2uint64_t(
            procfile_lineword(__pf_diskstats, l, 11)); // IOsInProgress

        curr_devstats->ios_total_ticks = str2uint64_t(
            procfile_lineword(__pf_diskstats, l, 12)); // IOsTotalTicks

        curr_devstats->weighted_io_ticks = str2uint64_t(
            procfile_lineword(__pf_diskstats, l, 13)); // WeightedIOTicks

        if (unlikely(pf_diskstats_line_words > 13)) {
            curr_devstats->dc_ios = str2uint64_t(
                procfile_lineword(__pf_diskstats, l, 14)); // DiscardIOs
            curr_devstats->dc_merges = str2uint64_t(
                procfile_lineword(__pf_diskstats, l, 15)); // DiscardMerges
            curr_devstats->dc_sectors = str2uint64_t(
                procfile_lineword(__pf_diskstats, l, 16)); // DiscardSectors
            curr_devstats->dc_ticks = str2uint64_t(
                procfile_lineword(__pf_diskstats, l, 17)); // DiscardTicks
        }

        // debug(
        //     "\ndiskstats[%d:%d]: %s rd_ios=%lu, rd_merges=%lu,
        //     rd_sectors=%lu, rd_ticks=%lu, wr_ios=%lu, wr_merges=%lu,
        //     wr_sectors=%lu, wr_ticks=%lu, ios_pgr=%lu, ios_total_ticks=%lu,
        //     weighted_io_ticks=%lu, dc_ios=%lu, dc_merges=%lu, dc_sectors=%lu,
        //     dc_ticks=%lu", dev->major, dev->minor, dev->device_name,
        //     curr_devstats->rd_ios, curr_devstats->rd_merges,
        //     curr_devstats->rd_sectors, curr_devstats->rd_ticks,
        //     curr_devstats->wr_ios, curr_devstats->wr_merges,
        //     curr_devstats->wr_sectors, curr_devstats->wr_ticks,
        //     curr_devstats->ios_pgr, curr_devstats->ios_total_ticks,
        //     curr_devstats->weighted_io_ticks, curr_devstats->dc_ios,
        //     curr_devstats->dc_merges, curr_devstats->dc_sectors,
        ...
        if (unlikely(0 == dev->curr_stats_id && 0 == dev->prev_stats_id)) {
            ...
            prom_counter_add(__metric_node_disk_write_spent_total_seconds,
                             (double)curr_devstats->wr_ticks * 0.001,
                             (const char *[]){ dev_name });

            prom_gauge_set(__metric_node_disk_io_now,
                           (double)curr_devstats->ios_pgr,
                           (const char *[]){ dev_name });
        ...
        debug("[PLUGIN_PROC:proc_diskstats] disk[%d:%d]:'%s' dt_sec=%.2f, "
              "dt=%lu",
              major, minor, dev_name, dt_sec, dt);
        ...
        double rd_ios_per_sec =
            ((double)(curr_devstats->rd_ios - prev_devstats->rd_ios)) / dt_sec;
        // IO每秒写次数(w/s)
        double wr_ios_per_sec =
            ((double)(curr_devstats->wr_ios - prev_devstats->wr_ios)) / dt_sec;

        // 每秒读取千字节数(rkB/s)
        double rd_kb_per_sec =
            ((double)(curr_devstats->rd_sectors - prev_devstats->rd_sectors))
            * dev->sector_size / 1024 / dt_sec;
        // 每秒写入千字节数(wkB/s)
        double wr_kb_per_sec =
            ((double)(curr_devstats->wr_sectors - prev_devstats->wr_sectors))
            * dev->sector_size / 1024 / dt_sec;
        ...
        double w_await = (curr_devstats->wr_ios - prev_devstats->wr_ios) ?
                             (curr_devstats->wr_ticks - prev_devstats->wr_ticks)
                                 / ((double)(curr_devstats->wr_ios
                                             - prev_devstats->wr_ios)) :
                             0.0;

        uint64_t curr_nr_ios = curr_devstats->rd_ios + curr_devstats->wr_ios
                               + curr_devstats->dc_ios;
        uint64_t prev_nr_ios = prev_devstats->rd_ios + prev_devstats->wr_ios
                               + prev_devstats->dc_ios;
        // 	每個I/O平均所需的時間（毫秒）
        double await =
        ...
        // in-flight）乘以自然时间
        double aqu_sz = ((double)(curr_devstats->weighted_io_ticks
                                  - prev_devstats->weighted_io_ticks))
                        / dt_sec / 1000.0;

        // 该硬盘设备的繁忙比率
        double utils = ((double)(curr_devstats->ios_total_ticks
                                 - prev_devstats->ios_total_ticks))
                       / dt_sec / 10.0;
        ...
        if (dev->dev_tp == DISK_TYPE_PHYSICAL) {
            ...
            debug("[PLUGIN_PROC:proc_diskstats] disk[%d:%d]:'%s' "
                  "system_read_kb=%ld(Kb ), "
                  ...
                  "wr_merges_per_sec=%.2f(wrqm/s), r_await=%.2f(ms), "
                  "w_await=%.2f(ms), "
                  "await=%.2f(ms), "
                  "aqu_sz=%.2f, areq_sz=%.2f, rareq_sz=%.2f, wareq_sz=%.2f， "
                  "utils=%.2f\n",
                  major, minor, dev_name, system_read_kb, system_write_kb,
                  rd_ios_per_sec, wr_ios_per_sec, rd_kb_per_sec, wr_kb_per_sec,
            ...
            debug("[PLUGIN_PROC:proc_diskstats] disk[%d:%d]:'%s' "
                  ...
                  "aqu_sz=%.2f, areq_sz=%.2f, rareq_sz=%.2f, wareq_sz=%.2f， "
                  "utils=%.2f\n",
                  major, minor, dev_name, rd_ios_per_sec, wr_ios_per_sec,
                  rd_kb_per_sec, wr_kb_per_sec, rd_merges_per_sec,
                  wr_merges_per_sec, r_await, w_await, await, aqu_sz, areq_sz,
                  rareq_sz, wareq_sz, utils);
        }
        ...
        prom_gauge_set(__metric_node_disk_io_now,
                       ...
                       (const char *[]){ dev_name });
        prom_counter_add(__metric_node_disk_io_spent_total_seconds,
                         (double)(curr_devstats->ios_total_ticks
                                  - prev_devstats->ios_total_ticks)
                             * 0.001,
                         (const char *[]){ dev_name });
        prom_counter_add(__metric_node_disk_io_time_weighted_total_seconds,
                         (double)(curr_devstats->weighted_io_ticks
                                  - prev_devstats->weighted_io_ticks)
                             * 0.001,
                         (const char *[]){ dev_name });
        ...
        prom_gauge_set(__metric_node_disk_iostat_tps, tps_per_sec,
                       (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_disk_iostat_read_requests_per_sec,
                       rd_ios_per_sec, (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_disk_iostat_write_requests_per_sec,
                       wr_ios_per_sec, (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_disk_iostat_read_kilobytes_per_sec,
                       rd_kb_per_sec, (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_disk_iostat_write_kilobytes_per_sec,
                       wr_kb_per_sec, (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_disk_iostat_read_requests_merged_per_sec,
                       rd_merges_per_sec, (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_disk_iostat_write_requests_merged_per_sec,
                       wr_merges_per_sec, (const char *[]){ dev_name });
        ...
        prom_gauge_set(__metric_node_disk_iostat_w_await_msecs, w_await,
                       (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_disk_iostat_await_msecs, await,
                       (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_disk_iostat_areq_sz, areq_sz,
                       (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_disk_iostat_rareq_sz, rareq_sz,
                       (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_disk_iostat_wareq_sz, wareq_sz,
                       (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_disk_iostat_aqu_sz, aqu_sz,
                       (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_disk_iostat_util, utils,
                       (const char *[]){ dev_name });
