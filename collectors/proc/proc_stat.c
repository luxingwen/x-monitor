...
int32_t init_collector_proc_stat() {
    ...
    if (unlikely(!__metric_node_interrupts_from_boot)) {
        __metric_node_interrupts_from_boot =
            prom_collector_registry_must_register_metric(
                prom_counter_new("node_interrupts",
                                 "CPU counts of interrupts serviced since boot "
                                 "time ",
                                 1, (const char *[]){ "cpu" }));
    }

    if (unlikely(!__metric_node_context_switches_from_boot)) {
        __metric_node_context_switches_from_boot =
            prom_collector_registry_must_register_metric(
                prom_counter_new("node_context_switches",
    ...
    if (unlikely(!__metric_node_processes_from_boot)) {
        __metric_node_processes_from_boot =
            prom_collector_registry_must_register_metric(
                prom_counter_new("node_procs_from_boot",
                                 "number of processes and threads created", 1,
                                 (const char *[]){ "cpu" }));
    }

    if (unlikely(!__metric_node_processes_running)) {
        __metric_node_processes_running =
    ...
    if (unlikely(!__metric_node_processes_blocked)) {
        __metric_node_processes_blocked =
            prom_collector_registry_must_register_metric(
                prom_gauge_new("node_procs_blocked",
                               "number of processes currently blocked, waiting "
                               "for I/O to complete",
                               1, (const char *[]){ "cpu" }));
    }

    if (unlikely(!__metric_node_cpu_jiffies)) {
        __metric_node_cpu_jiffies =
            prom_collector_registry_must_register_metric(prom_counter_new(
                "node_cpu_jiffies", "jiffies the cpus spent in each mode", 2,
                (const char *[]){ "cpu", "mode" }));
    }

    if (unlikely(!__metric_node_cpus_count)) {
        __metric_node_cpus_count = prom_collector_registry_must_register_metric(
            prom_gauge_new("node_cpus_count",
                           " The count of CPUs per node, useful for getting "
                           "CPU time as a percent of total.",
...
static void __do_cpu_utilization(size_t line, const char *cpu_label,
    ...
    debug("[PLUGIN_PROC:proc_stat] core_index: '%s' user_jiffies: %lu, "
          "nice_jiffies: %lu, "
          "system_jiffies: %lu, idle_jiffies: %lu, io_wait_jiffies: %lu, "
          "irq_jiffies: %lu, "
          "soft_irq_jiffies: %lu, steal_jiffies: %lu, guest_jiffies: %lu, "
          "guest_nice_jiffies: "
          "%lu",
          cpu_label, user_jiffies, nice_jiffies, system_jiffies, idle_jiffies,
    ...
    node_cpus_jiffies_total = user_jiffies + nice_jiffies + system_jiffies
                              ...
                              + soft_irq_jiffies + steal_jiffies;
    prom_counter_add(__metric_node_cpu_jiffies,
...
int32_t collector_proc_stat(int32_t UNUSED(update_every), usec_t UNUSED(dt),
    ...
    if (unlikely(!__cfg_proc_stat_filename)) {
        __cfg_proc_stat_filename = appconfig_get_member_str(
            config_path, "monitor_file", __def_proc_stat_filename);
    }

    if (unlikely(!__pf_stat)) {
        __pf_stat = procfile_open(__cfg_proc_stat_filename,
                                  " \t:", PROCFILE_FLAG_DEFAULT);
        if (unlikely(!__pf_stat)) {
            error("[PLUGIN_PROC:proc_stat] Cannot open %s",
    ...
    for (size_t index = 0; index < lines; index++) {
        ...
        if (likely(row_name[0] == 'c' && row_name[1] == 'p'
                   && row_name[2] == 'u')) {
            line_words = procfile_linewords(__pf_stat, index);

            if (unlikely(line_words < 10)) {
                error("Cannot read /proc/stat cpu line. Expected 10 params, "
                      "read %lu.",
                      line_words);
                continue;
            }
            ...
            // The first column is the total of all interrupts serviced; each
            // subsequent column is the total for that particular interrupt
            interrupts_from_boot =
                str2uint64_t(procfile_lineword(__pf_stat, index, 1));
            debug("[PLUGIN_PROC:proc_stat] interrupts_from_boot: %lu",
                  interrupts_from_boot);

            prom_counter_add(
                __metric_node_interrupts_from_boot,
                (double)(interrupts_from_boot
                         - proc_rds->stat_rdset.interrupts_from_boot),
                (const char *[]){ "intr" });
            proc_rds->stat_rdset.interrupts_from_boot = interrupts_from_boot;
        } else if (unlikely(strncmp(row_name, "ctxt", 4) == 0)) {
            // The "ctxt" line gives the total number of context switches across
            // all CPUs.
            ...
            // created, which includes (but is not limited to) those created by
            // calls to the fork() and clone() system calls.
            processes_from_boot =
                str2uint64_t(procfile_lineword(__pf_stat, index, 1));
            debug("[PLUGIN_PROC:proc_stat] processes_from_boot :%lu",
                  processes_from_boot);

            prom_counter_add(
                __metric_node_processes_from_boot,
                (double)(processes_from_boot
                         - proc_rds->stat_rdset.processes_from_boot),
            ...
            processes_running =
                str2uint64_t(procfile_lineword(__pf_stat, index, 1));
            debug("[PLUGIN_PROC:proc_stat] processes_running %lu",
                  processes_running);
            prom_gauge_set(__metric_node_processes_running,
                           (double)processes_running,
            ...
            processes_blocked =
                str2uint64_t(procfile_lineword(__pf_stat, index, 1));
            debug("[PLUGIN_PROC:proc_stat] procs_blocked: %lu",
                  processes_blocked);
            prom_gauge_set(__metric_node_processes_blocked,
                           (double)processes_blocked,
