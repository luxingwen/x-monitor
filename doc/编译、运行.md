...
2. 编译x-monitor
   ...
   ```
   mkdir build
   cmake3 ../ -DCMAKE_BUILD_TYPE=Debug -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
   ...
   ```
   cmake3 ../ -DCMAKE_BUILD_TYPE=Debug -DCMAKE_EXPORT_COMPILE_COMMANDS=ON -DAPP_METRIC_TAG=dbapp
   ```
...
5. 源码统计
   ...
   ```
   find . -path ./extra -prune -o -path ./plugin_ebpf/usr -prune -o -path ./plugin_ebpf/bpf/.output -prune -o  -name "*.[ch]"|xargs wc -l
   ```
