...
- 聚合操作符：PromQL提供的聚合操作符用于聚合单个即时向量的所有时间序列列表，把聚合的结果存入到新生成的一条时间序列向量中。
...
- node_schedstat_timeslices_total
  ...
  说明：Number of timeslices executed by CPU. 本CPU运行了多少时间片

- node_schedstat_waiting_seconds_total

  说明：Number of seconds spent by processing waiting for this CPU. 本处理器任务等待运行的时间

- node_schedstat_running_seconds_total

  说明：Number of seconds CPU spent running a process. 本处理器运行任务的总时间
...
- node_sockstat_TCP_mem
  ...
  说明：socket buffer usage (unit page. Measured with scp, when the speed is 4803.9kB/s: its value = 11, the corresponding 22 port in netstat -ant Recv-Q = 0, Send-Q ≈400)，KiB. 套接字缓冲区使用量，单位是page
...
- node_sockstat_UDP_mem
  ...
  说明：IPv4 UDP Sockets Memory, 单位是page，udp使用的物理内存大小。
