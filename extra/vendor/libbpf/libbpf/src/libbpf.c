...
__printf(2, 3) void libbpf_print(enum libbpf_print_level level,
                                 const char *format, ...) {
    va_list args;
    int old_errno;
...
static bool insn_is_subprog_call(const struct bpf_insn *insn) {
    return BPF_CLASS(insn->code) == BPF_JMP && BPF_OP(insn->code) == BPF_CALL
           && BPF_SRC(insn->code) == BPF_K && insn->src_reg == BPF_PSEUDO_CALL
           && insn->dst_reg == 0 && insn->off == 0;
}
...
static int bpf_object__add_programs(struct bpf_object *obj, Elf_Data *sec_data,
    ...
    for (i = 0; i < nr_syms; i++) {
        ...
        if (sec_off + prog_sz > sec_sz) {
            pr_warn("sec '%s': program at offset %zu crosses section "
                    "boundary\n",
                    sec_name, sec_off);
            return -LIBBPF_ERRNO__FORMAT;
        }
        ...
        pr_debug("sec '%s': found program '%s' at insn offset %zu (%zu bytes), "
                 ...
                 sec_name, name, sec_off / BPF_INSN_SZ, sec_off,
                 prog_sz / BPF_INSN_SZ, prog_sz);
        ...
        err = bpf_object__init_prog(obj, prog, name, sec_idx, sec_name, sec_off,
...
static int find_struct_ops_kern_types(const struct btf *btf, const char *tname,
     ...
     * btf_vmlinux.
     */
    kern_vtype_id = find_btf_by_prefix_kind(btf, STRUCT_OPS_VALUE_PREFIX, tname,
                                            BTF_KIND_STRUCT);
    if (kern_vtype_id < 0) {
        pr_warn("struct_ops init_kern: struct %s%s is not found in kernel "
                "BTF\n",
...
static int bpf_map__init_kern_struct_ops(struct bpf_map *map,
    ...
    pr_debug("struct_ops init_kern %s: type_id:%u kern_type_id:%u "
             "kern_vtype_id:%u\n",
    ...
    for (i = 0; i < btf_vlen(type); i++, member++) {
        ...
        if (!kern_member) {
            pr_warn("struct_ops init_kern %s: Cannot find member %s in kernel "
                    "BTF\n",
                    map->name, mname);
            return -ENOTSUP;
        }
        ...
        kern_mtype =
            skip_mods_and_typedefs(kern_btf, kern_member->type, &kern_mtype_id);
        if (BTF_INFO_KIND(mtype->info) != BTF_INFO_KIND(kern_mtype->info)) {
            pr_warn("struct_ops init_kern %s: Unmatched member type %s %u != "
                    "%u(kernel)\n",
        ...
        if (btf_is_ptr(mtype)) {
            ...
            kern_mtype = skip_mods_and_typedefs(kern_btf, kern_mtype->type,
                                                &kern_mtype_id);
            ...
            if (!btf_is_func_proto(kern_mtype)) {
                pr_warn("struct_ops init_kern %s: kernel member %s is not a "
                        "func ptr\n",
                        map->name, mname);
                return -ENOTSUP;
            }
            ...
            pr_debug("struct_ops init_kern %s: func ptr %s is set to prog %s "
                     "from data(+%u) to kern_data(+%u)\n",
        ...
        if (msize < 0 || kern_msize < 0 || msize != kern_msize) {
            pr_warn("struct_ops init_kern %s: Error in size of member %s: %zd "
                    "!= %zd(kernel)\n",
                    map->name, mname, (ssize_t)msize, (ssize_t)kern_msize);
            return -ENOTSUP;
        }

        pr_debug("struct_ops init_kern %s: copy %s %u bytes from data(+%u) to "
                 "kern_data(+%u)\n",
...
static int bpf_object__init_struct_ops_maps(struct bpf_object *obj) {
    ...
    btf = obj->btf;
    datasec_id = btf__find_by_name_kind(btf, STRUCT_OPS_SEC, BTF_KIND_DATASEC);
    if (datasec_id < 0) {
    ...
    for (i = 0; i < btf_vlen(datasec); i++, vsi++) {
        ...
        if (type_id < 0) {
            pr_warn("struct_ops init: Cannot resolve var type_id %u in DATASEC "
                    "%s\n",
        ...
        st_ops->kern_func_off =
            malloc(btf_vlen(type) * sizeof(*st_ops->kern_func_off));
        ...
        memcpy(st_ops->data, obj->efile.st_ops_data->d_buf + vsi->offset,
               type->size);
        st_ops->tname = tname;
        st_ops->type = type;
        st_ops->type_id = type_id;

        pr_debug("struct_ops init: struct %s(type_id=%u) %s found at offset "
                 "%u\n",
...
static struct bpf_object *bpf_object__new(const char *path, const void *obj_buf,
                                          size_t obj_buf_sz,
                                          const char *obj_name) {
    struct bpf_object *obj;
    char *end;
...
static bool bpf_map_type__is_map_in_map(enum bpf_map_type type) {
    if (type == BPF_MAP_TYPE_ARRAY_OF_MAPS || type == BPF_MAP_TYPE_HASH_OF_MAPS)
        return true;
    return false;
}
...
static char *internal_map_name(struct bpf_object *obj, const char *real_name) {
    ...
    if (strchr(real_name + 1, '.') != NULL)
        ...
        pfx_len =
            min((size_t)BPF_OBJ_NAME_LEN - sfx_len - 1, strlen(obj->name));
...
static int bpf_object__init_internal_map(struct bpf_object *obj,
                                         ...
                                         const char *real_name, int sec_idx,
...
static int bpf_object__init_global_data_maps(struct bpf_object *obj) {
    ...
    for (sec_idx = 1; sec_idx < obj->efile.sec_cnt; sec_idx++) {
        ...
        case SEC_DATA:
            ...
            err = bpf_object__init_internal_map(obj, LIBBPF_MAP_DATA, sec_name,
                                                sec_idx, sec_desc->data->d_buf,
        ...
        case SEC_RODATA:
            ...
            sec_name = elf_sec_name(obj, elf_sec_by_idx(obj, sec_idx));
            err =
                bpf_object__init_internal_map(obj, LIBBPF_MAP_RODATA, sec_name,
                                              sec_idx, sec_desc->data->d_buf,
                                              sec_desc->data->d_size);
            break;
        case SEC_BSS:
            sec_name = elf_sec_name(obj, elf_sec_by_idx(obj, sec_idx));
            err = bpf_object__init_internal_map(obj, LIBBPF_MAP_BSS, sec_name,
                                                sec_idx, NULL,
                                                sec_desc->data->d_size);
            break;
        default:
...
static int set_kcfg_value_tri(struct extern_desc *ext, void *ext_val,
    ...
    case KCFG_BOOL:
        if (value == 'm') {
            pr_warn("extern (kcfg) '%s': value '%c' implies tristate or char "
                    "type\n",
                    ext->name, value);
            return -EINVAL;
        }
    ...
    case KCFG_CHAR_ARR:
    default:
        pr_warn("extern (kcfg) '%s': value '%c' implies bool, tristate, or "
                "char type\n",
                ext->name, value);
        return -EINVAL;
    }
...
static int set_kcfg_value_str(struct extern_desc *ext, char *ext_val,
    ...
    len -= 2;
    if (len >= ext->kcfg.sz) {
        pr_warn("extern (kcfg) '%s': long string '%s' of (%zu bytes) truncated "
                "to %d bytes\n",
...
static int set_kcfg_value_num(struct extern_desc *ext, void *ext_val,
    ...
    if (ext->kcfg.type != KCFG_INT && ext->kcfg.type != KCFG_CHAR
        ...
        pr_warn("extern (kcfg) '%s': value '%llu' implies integer, char, or "
                "boolean type\n",
...
static int bpf_object__process_kconfig_line(struct bpf_object *obj, char *buf,
                                            void *data) {
...
int parse_btf_map_def(const char *map_name, struct btf *btf,
                      ...
                      struct btf_map_def *map_def,
                      struct btf_map_def *inner_def) {
    const struct btf_type *t;
    ...
    for (i = 0; i < vlen; i++, m++) {
        ...
        if (strcmp(name, "type") == 0) {
            ...
            if (sz < 0) {
                pr_warn("map '%s': can't determine key size for type [%u]: "
                        "%zd.\n",
            ...
            sz = btf__resolve_size(btf, t->type);
            if (sz < 0) {
                pr_warn("map '%s': can't determine value size for type [%u]: "
                        "%zd.\n",
                        map_name, t->type, (ssize_t)sz);
                return sz;
            }
            ...
            if (is_prog_array) {
                if (!btf_is_func_proto(t)) {
                    pr_warn("map '%s': prog-array value def is of unexpected "
                            "kind %s.\n",
            ...
            if (!btf_is_struct(t)) {
                pr_warn("map '%s': map-in-map inner def is of unexpected kind "
                        "%s.\n",
                        map_name, btf_kind_str(t));
                return -EINVAL;
            }
            ...
            snprintf(inner_map_name, sizeof(inner_map_name), "%s.inner",
                     map_name);
            err = parse_btf_map_def(inner_map_name, btf, t, strict, inner_def,
                                    NULL);
            if (err)
                return err;
...
static void fill_map_from_def(struct bpf_map *map,
    ...
    if (def->parts & MAP_DEF_KEY_TYPE)
        pr_debug("map '%s': found key [%u], sz = %u.\n", map->name,
                 def->key_type_id, def->key_size);
    else if (def->parts & MAP_DEF_KEY_SIZE)
        pr_debug("map '%s': found key_size = %u.\n", map->name, def->key_size);

    if (def->parts & MAP_DEF_VALUE_TYPE)
        pr_debug("map '%s': found value [%u], sz = %u.\n", map->name,
                 def->value_type_id, def->value_size);
...
static int bpf_object__init_user_btf_map(struct bpf_object *obj,
    ...
    pr_debug("map '%s': at sec_idx %d, offset %zu.\n", map_name, map->sec_idx,
             map->sec_offset);

    err = parse_btf_map_def(map->name, obj->btf, def, strict, &map_def,
                            &inner_def);
    if (err)
        return err;
...
static int bpf_object__init_user_btf_maps(struct bpf_object *obj, bool strict,
    ...
    for (i = 0; i < vlen; i++) {
        err = bpf_object__init_user_btf_map(obj, sec, i,
                                            obj->efile.btf_maps_shndx, data,
                                            strict, pin_root_path);
        if (err)
            return err;
    }
            ...
            *(int *)(t + 1) = BTF_INT_ENC(0, 0, 8);
            ...
            if (enum64_placeholder_id == 0) {
                enum64_placeholder_id =
                    btf__add_int(btf, "enum64_placeholder", 1, 0);
                if (enum64_placeholder_id < 0)
...
static int bpf_object__init_btf(struct bpf_object *obj, Elf_Data *btf_data,
    ...
    if (btf_ext_data) {
        ...
        if (!obj->btf) {
            pr_debug("Ignore ELF section %s because its depending ELF section "
                     "%s is not found.\n",
...
static int btf_fixup_datasec(struct bpf_object *obj, struct btf *btf,
    ...
    if (t->size == 0) {
        ...
        if (err || !size) {
            pr_debug("sec '%s': failed to determine size from ELF: size %u, "
                     "err %d\n",
                     sec_name, size, err);
            return -ENOENT;
        }
...
static int bpf_object__sanitize_and_load_btf(struct bpf_object *obj) {
    ...
    if (obj->gen_loader) {
         ...
         * This fd == 0 will not be used with any syscall and will be reset to
         * -1 eventually.
         */
...
static int bpf_object__elf_collect(struct bpf_object *obj) {
    ...
    if (!obj->efile.symbols) {
        pr_warn("elf: couldn't find symbol table in %s, stripped object "
                "file?\n",
                obj->path);
        return -ENOENT;
    }
    ...
    while ((scn = elf_nextscn(elf, scn)) != NULL) {
        ...
        pr_debug("elf: section(%d) %s, size %ld, link %d, flags %lx, type=%d\n",
                 idx, name, (unsigned long)data->d_size, (int)sh->sh_link,
                 (unsigned long)sh->sh_flags, (int)sh->sh_type);
        ...
        if (strcmp(name, "license") == 0) {
            ...
            pr_warn("elf: legacy map definitions in 'maps' section are not "
            ...
            if (!section_have_execinstr(obj, targ_sec_idx)
                ...
                pr_info("elf: skipping relo section(%d) %s for section(%d) "
                        "%s\n",
     ...
     */
    if (obj->nr_programs)
        qsort(obj->programs, obj->nr_programs, sizeof(*obj->programs),
              cmp_progs);
...
static int find_extern_btf_id(const struct btf *btf, const char *ext_name) {
    ...
    for (i = 1; i < n; i++) {
        ...
        if (btf_is_var(t) && btf_var(t)->linkage != BTF_VAR_GLOBAL_EXTERN)
            return -EINVAL;
...
static int add_dummy_ksym_var(struct btf *btf) {
    ...
    dummy_var_btf_id =
        btf__add_var(btf, "dummy_ksym", BTF_VAR_GLOBAL_ALLOCATED, int_btf_id);
...
static int bpf_object__collect_externs(struct bpf_object *obj) {
    ...
    for (i = 0; i < n; i++) {
        ...
        if (strcmp(sec_name, KCONFIG_SEC) == 0) {
            ...
            if (ext->kcfg.align <= 0) {
                pr_warn("failed to determine alignment of extern (kcfg) '%s': "
                        "%d\n",
                        ext_name, ext->kcfg.align);
                return -EINVAL;
            }
            ext->kcfg.type =
                find_kcfg_type(obj->btf, t->type, &ext->kcfg.is_signed);
    ...
    if (kcfg_sec) {
        ...
        for (i = 0; i < obj->nr_extern; i++) {
            ...
            pr_debug("extern (kcfg) #%d: symbol %d, off %u, name %s\n", i,
                     ext->sym_idx, ext->kcfg.data_off, ext->name);
        }
        sec->size = off;
        n = btf_vlen(sec);
        for (i = 0; i < n; i++) {
...
bpf_object__section_to_libbpf_map_type(const struct bpf_object *obj,
                                       int shndx) {
...
static int bpf_program__record_reloc(struct bpf_program *prog,
    ...
    if (!is_call_insn(insn) && !is_ldimm64_insn(insn)) {
        pr_warn("prog '%s': invalid relo against '%s' for insns[%d].code "
                "0x%x\n",
    ...
    if (sym_is_extern(sym)) {
        ...
        if (i >= n) {
            pr_warn("prog '%s': extern relo failed to find extern for '%s' "
                    "(%d)\n",
    ...
    if (sym_is_subprog(sym, obj->efile.text_shndx)) {
        ...
        if ((sym->st_value % BPF_INSN_SZ) || (insn->imm % BPF_INSN_SZ)) {
            pr_warn("prog '%s': bad subprog addr relo against '%s' at offset "
                    "%zu+%d\n",
    ...
    if (type == LIBBPF_MAP_UNSPEC) {
        ...
        for (map_idx = 0; map_idx < nr_maps; map_idx++) {
            ...
            if (map->libbpf_type != type || map->sec_idx != sym->st_shndx
                ...
                continue;
            pr_debug("prog '%s': found map %zd (%s, sec %d, off %zu) for insn "
                     "#%u\n",
        ...
        if (map_idx >= nr_maps) {
            pr_warn("prog '%s': map relo failed to find map for section '%s', "
                    "off %zu\n",
    ...
    for (map_idx = 0; map_idx < nr_maps; map_idx++) {
        ...
        pr_debug("prog '%s': found data map %zd (%s, sec %d, off %zu) for insn "
                 "%u\n",
                 prog->name, map_idx, map->name, map->sec_idx, map->sec_offset,
                 insn_idx);
        break;
    }
    if (map_idx >= nr_maps) {
...
static int bpf_object__collect_prog_relos(struct bpf_object *obj,
    ...
    for (i = 0; i < nrels; i++) {
        ...
        if (sym->st_shndx >= obj->efile.sec_cnt) {
            pr_warn("sec '%s': corrupted symbol #%zu pointing to invalid "
                    "section #%zu for relo #%d\n",
        ...
        if (!prog) {
            pr_debug("sec '%s': relo #%d: couldn't find program in section "
                     "'%s' for insn #%u, probably overridden weak function, "
                     "skipping...\n",
                     relo_sec_name, i, sec_name, insn_idx);
            continue;
        }

        relos = libbpf_reallocarray(prog->reloc_desc, prog->nr_reloc + 1,
                                    sizeof(*relos));
        if (!relos)
            return -ENOMEM;
        ...
        err = bpf_program__record_reloc(prog, &relos[prog->nr_reloc], insn_idx,
                                        sym_name, sym, rel);
        if (err)
            return err;
...
int bpf_map__reuse_fd(struct bpf_map *map, int fd) {
    ...
    if (name_len == BPF_OBJ_NAME_LEN - 1
        && strncmp(map->name, info.name, name_len) == 0)
...
static int bpf_object__probe_loading(struct bpf_object *obj) {
    ...
    if (ret)
        pr_warn("Failed to bump RLIMIT_MEMLOCK (err = %d), you might need to "
                "do it explicitly!\n",
                ret);
...
static int probe_kern_global_data(void) {
    ...
    map = bpf_map_create(BPF_MAP_TYPE_ARRAY, "libbpf_global", sizeof(int), 32,
                         1, NULL);
    if (map < 0) {
        ret = -errno;
        cp = libbpf_strerror_r(ret, errmsg, sizeof(errmsg));
...
static int probe_kern_btf(void) {
    ...
    return probe_fd(
        libbpf__load_raw_btf((char *)types, sizeof(types), strs, sizeof(strs)));
}
...
static int probe_kern_array_mmap(void) {
    ...
    fd = bpf_map_create(BPF_MAP_TYPE_ARRAY, "libbpf_mmap", sizeof(int),
                        sizeof(int), 1, &opts);
    return probe_fd(fd);
}
...
static int probe_kern_exp_attach_type(void) {
     ...
     */
    fd = bpf_prog_load(BPF_PROG_TYPE_CGROUP_SOCK, NULL, "GPL", insns, insn_cnt,
                       &opts);
    return probe_fd(fd);
}
...
static int probe_kern_probe_read_kernel(void) {
    ...
    fd = bpf_prog_load(BPF_PROG_TYPE_TRACEPOINT, NULL, "GPL", insns, insn_cnt,
                       NULL);
    return probe_fd(fd);
}
...
static int probe_prog_bind_map(void) {
    ...
    map = bpf_map_create(BPF_MAP_TYPE_ARRAY, "libbpf_det_bind", sizeof(int), 32,
                         1, NULL);
    if (map < 0) {
        ret = -errno;
        cp = libbpf_strerror_r(ret, errmsg, sizeof(errmsg));
...
static int probe_perf_link(void) {
    ...
    prog_fd = bpf_prog_load(BPF_PROG_TYPE_TRACEPOINT, NULL, "GPL", insns,
...
static int probe_kern_bpf_cookie(void) {
    ...
    ret =
        bpf_prog_load(BPF_PROG_TYPE_KPROBE, NULL, "GPL", insns, insn_cnt, NULL);
    return probe_fd(ret);
}
...
static int bpf_object__populate_internal_map(struct bpf_object *obj,
    ...
    if (obj->gen_loader) {
        bpf_gen__map_update_elem(obj->gen_loader, map - obj->maps, map->mmaped,
                                 map->def.value_size);
...
static int bpf_object__create_map(struct bpf_object *obj, struct bpf_map *map,
    ...
    if (obj->gen_loader) {
        bpf_gen__map_create(obj->gen_loader, def->type, map_name, def->key_size,
                            def->value_size, def->max_entries, &create_attr,
                            is_inner ? -1 : map - obj->maps);
        /* Pretend to have valid FD to pass various fd >= 0 checks.
         * This fd == 0 will not be used with any syscall and will be reset to
         * -1 eventually.
         */
        ...
        map->fd =
            bpf_map_create(def->type, map_name, def->key_size, def->value_size,
    ...
    if (map->fd < 0
        ...
        err = -errno;
        cp = libbpf_strerror_r(err, errmsg, sizeof(errmsg));
        pr_warn("Error in bpf_create_map_xattr(%s):%s(%d). Retrying without "
                "BTF.\n",
                map->name, cp, err);
        create_attr.btf_fd = 0;
        create_attr.btf_key_type_id = 0;
...
static int init_map_in_map_slots(struct bpf_object *obj, struct bpf_map *map) {
    ...
    for (i = 0; i < map->init_slots_sz; i++) {
        ...
        if (obj->gen_loader) {
            bpf_gen__populate_outer_map(obj->gen_loader, map - obj->maps, i,
                                        targ_map - obj->maps);
        } else {
        ...
        if (err) {
            err = -errno;
            pr_warn("map '%s': failed to initialize slot [%d] to map '%s' "
                    "fd=%d: %d\n",
...
static int init_prog_array_slots(struct bpf_object *obj, struct bpf_map *map) {
    ...
    for (i = 0; i < map->init_slots_sz; i++) {
        ...
        if (err) {
            err = -errno;
            pr_warn("map '%s': failed to initialize slot [%d] to prog '%s' "
                    "fd=%d: %d\n",
...
int bpf_core_add_cands(struct bpf_core_cand *local_cand,
    ...
    for (i = targ_start_id; i < n; i++) {
        ...
        pr_debug("CO-RE relocating [%d] %s %s: found target candidate [%d] %s "
                 "%s in [%s]\n",
                 local_cand->id, btf_kind_str(local_t), local_name, i,
                 btf_kind_str(t), targ_name, targ_btf_name);
...
static int load_module_btfs(struct bpf_object *obj) {
    ...
    while (true) {
        ...
        err = libbpf_ensure_mem((void **)&obj->btf_modules,
                                &obj->btf_module_cap, sizeof(*obj->btf_modules),
                                obj->btf_module_cnt + 1);
        if (err)
            goto err_out;
...
bpf_core_find_cands(struct bpf_object *obj, const struct btf *local_btf,
    ...
    for (i = 0; i < obj->btf_module_cnt; i++) {
        err = bpf_core_add_cands(&local_cand, local_essent_len,
                                 ...
                                 btf__type_cnt(obj->btf_vmlinux), cands);
        if (err)
            goto err_out;
    }
...
static int bpf_core_resolve_relo(struct bpf_program *prog,
    ...
    if (relo->kind != BPF_CORE_TYPE_ID_LOCAL
        ...
        if (IS_ERR(cands)) {
            pr_warn("prog '%s': relo #%d: target candidate search failed for "
                    "[%d] %s %s: %ld\n",
    ...
    for_each_btf_ext_sec(seg, sec) {
        ...
        for_each_btf_ext_rec(seg, sec, i, rec) {
            ...
            if (!prog) {
                 ...
                 */
                pr_debug("sec '%s': skipping CO-RE relocation #%d for insn #%d "
                         "belonging to eliminated weak subprogram\n",
                         sec_name, i, insn_idx);
                continue;
            }
            ...
            if (err) {
                pr_warn("prog '%s': relo #%d: failed to record relocation: "
                        "%d\n",
                        prog->name, i, err);
                goto out;
            }
            ...
            err = bpf_core_resolve_relo(prog, rec, i, obj->btf, cand_cache,
                                        &targ_res);
            if (err) {
            ...
            err = bpf_core_patch_insn(prog->name, insn, insn_idx, rec, i,
                                      &targ_res);
            if (err) {
...
static void poison_map_ldimm64(struct bpf_program *prog, int relo_idx,
                               int insn_idx, struct bpf_insn *insn, int map_idx,
    ...
    for (i = 0; i < prog->nr_reloc; i++) {
        ...
        case RELO_LD64:
            ...
            if (obj->gen_loader) {
                ...
                poison_map_ldimm64(prog, i, relo->insn_idx, insn, relo->map_idx,
                                   map);
            }
            break;
        case RELO_DATA:
...
static int reloc_prog_func_and_line_info(const struct bpf_object *obj,
    ...
    if (err) {
        ...
        pr_warn("prog '%s': missing .BTF.ext function info for the main "
                "program, skipping all of .BTF.ext func info.\n",
                prog->name);
    }
...
line_info:
    ...
    if (err) {
        ...
        /* Have problem loading the very first info. Ignore the rest. */
        pr_warn("prog '%s': missing .BTF.ext line info for the main program, "
                "skipping all of .BTF.ext line info.\n",
                prog->name);
    }
    return 0;
}
...
static int bpf_object__reloc_code(struct bpf_object *obj,
    ...
    for (insn_idx = 0; insn_idx < prog->sec_insn_cnt; insn_idx++) {
        ...
        subprog =
            find_prog_by_sec_insn(obj, obj->efile.text_shndx, sub_insn_idx);
        if (!subprog) {
            pr_warn("prog '%s': no .text section found yet sub-program call "
                    "exists\n",
                    prog->name);
            return -LIBBPF_ERRNO__RELOC;
        }
        ...
        if (subprog->sub_insn_off == 0) {
            ...
            insns =
                libbpf_reallocarray(main_prog->insns, new_cnt, sizeof(*insns));
            if (!insns) {
        ...
        pr_debug("prog '%s': insn #%zu relocated, imm %d points to subprog "
                 "'%s' (now at %zu offset)\n",
                 prog->name, insn_idx, insn->imm, subprog->name,
...
static int bpf_object__collect_map_relos(struct bpf_object *obj,
    ...
    for (i = 0; i < nrels; i++) {
        ...
        if (j == obj->nr_maps) {
            pr_warn(".maps relo #%d: cannot find map '%s' at rel->r_offset "
                    "%zu\n",
        ...
        if (is_map_in_map) {
            ...
            if (map->def.type == BPF_MAP_TYPE_HASH_OF_MAPS
                && map->def.key_size != sizeof(int)) {
                pr_warn(".maps relo #%d: hash-of-maps '%s' should have key "
                        "size %zu.\n",
            ...
            if (!targ_prog) {
                pr_warn(".maps relo #%d: '%s' isn't a valid program "
                        "reference\n",
                        i, name);
                return -ESRCH;
            }
...
static bool insn_is_helper_call(struct bpf_insn *insn,
    ...
    if (BPF_CLASS(insn->code) == BPF_JMP && BPF_OP(insn->code) == BPF_CALL
        && BPF_SRC(insn->code) == BPF_K && insn->src_reg == 0
        && insn->dst_reg == 0) {
        *func_id = insn->imm;
        return true;
    }
...
static int libbpf_find_attach_btf_id(struct bpf_program *prog,
                                     const char *attach_name, int *btf_obj_fd,
                                     int *btf_type_id);
...
static int libbpf_prepare_prog_load(struct bpf_program *prog,
    ...
    if ((def & SEC_ATTACH_BTF) && !prog->attach_btf_id) {
        ...
        if (!attach_name) {
             ...
             * correctness anyways.
             */
            pr_warn("prog '%s': no BTF-based attach target is specified, use "
...
static int bpf_object_load_prog(struct bpf_object *obj,
    ...
    if (prog->type == BPF_PROG_TYPE_UNSPEC) {
         ...
         */
        pr_warn("prog '%s': missing BPF prog type, check ELF section name "
                "'%s'\n",
    ...
    if (obj->gen_loader) {
        bpf_gen__prog_load(obj->gen_loader, prog->type, prog->name, license,
                           insns, insns_cnt, &load_attr, prog - obj->programs);
        *prog_fd = -1;
        return 0;
    }
...
static void patch_log(char *buf, size_t buf_sz, size_t log_sz, char *orig,
                      size_t orig_sz, const char *patch) {
    /* size of the remaining log content to the right from the to-be-replaced
     * part */
...
static void fixup_log_failed_core_relo(struct bpf_program *prog, char *buf,
                                       size_t buf_sz, size_t log_sz,
...
static void fixup_log_missing_map_load(struct bpf_program *prog, char *buf,
                                       size_t buf_sz, size_t log_sz,
                                       char *line1, char *line2, char *line3) {
    /* Expected log for failed and not properly guarded CO-RE relocation:
...
static void fixup_verifier_log(struct bpf_program *prog, char *buf,
    ...
    for (i = 0; i < max_last_line_cnt; i++, next_line = cur_line) {
        ...
        if (str_has_pfx(cur_line, "invalid func unknown#195896080\n")) {
            ...
            fixup_log_failed_core_relo(prog, buf, buf_sz, log_sz, prev_line,
                                       cur_line, next_line);
            return;
        } else if (str_has_pfx(cur_line, "invalid func "
                                         "unknown#" MAP_LDIMM64_POISON_PFX)) {
            prev_line = find_prev_line(buf, cur_line);
            if (!prev_line)
                continue;

            fixup_log_missing_map_load(prog, buf, buf_sz, log_sz, prev_line,
                                       cur_line, next_line);
            return;
        }
    }
...
bpf_object_open(const char *path, const void *obj_buf, size_t obj_buf_sz,
                const struct bpf_object_open_opts *opts) {
    ...
    if (obj_buf) {
        if (!obj_name) {
            snprintf(tmp_name, sizeof(tmp_name), "%lx-%lx",
                     (unsigned long)obj_buf, (unsigned long)obj_buf_sz);
            obj_name = tmp_name;
        }
        path = obj_name;
...
static int kallsyms_cb(unsigned long long sym_addr, char sym_type,
    ...
    if (ext->is_set && ext->ksym.addr != sym_addr) {
        pr_warn("extern (ksym) '%s': resolution is ambiguous: 0x%llx or "
                "0x%llx\n",
...
static int bpf_object__resolve_ksym_var_btf_id(struct bpf_object *obj,
    ...
    if (err <= 0) {
        ...
        pr_warn("extern (var ksym) '%s': incompatible types, expected [%d] %s "
                ...
                ext->name, local_type_id, btf_kind_str(local_type), local_name,
                targ_type_id, btf_kind_str(targ_type), targ_name);
        return -EINVAL;
    }
...
static int bpf_object__resolve_ksym_func_btf_id(struct bpf_object *obj,
    ...
    kfunc_id =
        find_ksym_btf_id(obj, ext->name, BTF_KIND_FUNC, &kern_btf, &mod_btf);
    if (kfunc_id < 0) {
    ...
    ret = bpf_core_types_are_compat(obj->btf, local_func_proto_id, kern_btf,
                                    kfunc_proto_id);
    if (ret <= 0) {
        pr_warn("extern (func ksym) '%s': func_proto [%d] incompatible with "
                "kernel [%d]\n",
    ...
    if (mod_btf && !mod_btf->fd_array_idx) {
        ...
        if (obj->fd_array_cnt == INT16_MAX) {
            pr_warn("extern (func ksym) '%s': module BTF fd index %d too big "
                    "to fit in bpf_insn offset\n",
    ...
    for (i = 0; i < obj->nr_extern; i++) {
        ...
        if (ext->type == EXT_KSYM) {
            ...
            if (strcmp(ext->name, "LINUX_KERNEL_VERSION") == 0) {
                ...
                if (!value) {
                    pr_warn("extern (kcfg) '%s': failed to get kernel "
                            "version\n",
                            ext->name);
                    return -EINVAL;
                }
                 ...
                 * __kconfig externs, where LINUX_ ones are virtual and filled
                 * out customly by libbpf (their values don't come from
                 * Kconfig). If LINUX_xxx variable is not recognized by libbpf,
                 * but is marked
...
static int bpf_object_load(struct bpf_object *obj, int extra_log_level,
    ...
    if (obj->gen_loader) {
        ...
        for (i = 0; i < obj->nr_maps; i++)
            obj->maps[i].fd = -1;
        if (!err)
            err = bpf_gen__finish(obj->gen_loader, obj->nr_programs,
                                  obj->nr_maps);
    }
...
__alias(bpf_map__pin_path) const
    char *bpf_map__get_pin_path(const struct bpf_map *map);
...
__alias(bpf_program__type) enum bpf_prog_type
...
#define SEC_DEF(sec_pfx, ptype, atype, flags, ...)                    \
    {                                                                 \
        .sec = (char *)sec_pfx, .prog_type = BPF_PROG_TYPE_##ptype,   \
        .expected_attach_type = atype, .cookie = (long)(flags),       \
        .prog_prepare_load_fn = libbpf_prepare_prog_load, __VA_ARGS__ \
    }
...
int libbpf_unregister_prog_handler(int handler_id) {
    ...
    if (has_custom_fallback_def
        && custom_fallback_def.handler_id == handler_id) {
...
static int bpf_object__collect_st_ops_relos(struct bpf_object *obj,
    ...
    for (i = 0; i < nrels; i++) {
        ...
        st_ops = map->st_ops;
        pr_debug("struct_ops reloc %s: for %lld value %lld shdr_idx %u "
                 "rel->r_offset %zu map->sec_offset %zu name %d (\'%s\')\n",
                 map->name, (long long)(rel->r_info >> 32),
                 (long long)sym->st_value, shdr_idx, (size_t)rel->r_offset,
                 map->sec_offset, sym->st_name, name);

        if (shdr_idx >= SHN_LORESERVE) {
            pr_warn("struct_ops reloc %s: rel->r_offset %zu shdr_idx %u "
                    "unsupported non-static function\n",
        ...
        if (!prog) {
            pr_warn("struct_ops reloc %s: cannot find prog at shdr_idx %u to "
                    "relocate func ptr %s\n",
        ...
        if (prog->type != BPF_PROG_TYPE_STRUCT_OPS) {
            pr_warn("struct_ops reloc %s: prog %s is not struct_ops BPF "
                    "program\n",
                    map->name, prog->name);
            return -EINVAL;
        }
        ...
        if (prog->attach_btf_id != st_ops->type_id
            || prog->expected_attach_type != member_idx) {
            pr_warn("struct_ops reloc %s: cannot use prog %s in sec %s with "
                    "type %u attach_btf_id %u expected_attach_type %u for func "
                    "ptr %s\n",
...
static int find_kernel_btf_id(struct bpf_object *obj, const char *attach_name,
                              enum bpf_attach_type attach_type, int *btf_obj_fd,
                              int *btf_type_id) {
    int ret, i;
...
static int libbpf_find_attach_btf_id(struct bpf_program *prog,
    ...
    if (prog->type == BPF_PROG_TYPE_EXT || attach_prog_fd) {
        ...
        if (err < 0) {
            pr_warn("prog '%s': failed to find BPF program (FD %d) BTF ID for "
                    "'%s': %d\n",
...
int bpf_map__set_initial_value(struct bpf_map *map, const void *data,
                               size_t size) {
...
static int validate_map_op(const struct bpf_map *map, size_t key_sz,
    ...
    case BPF_MAP_TYPE_PERCPU_CGROUP_STORAGE: {
        ...
        if (value_sz != num_cpu * elem_sz) {
            pr_warn("map '%s': unexpected value size %zu provided for per-CPU "
                    "map, expected %d * %zu = %zd\n",
    ...
    default:
        if (map->def.value_size != value_sz) {
            pr_warn("map '%s': unexpected value size %zu provided, expected "
                    "%u\n",
...
int bpf_map__delete_elem(const struct bpf_map *map, const void *key,
...
int bpf_map__lookup_and_delete_elem(const struct bpf_map *map, const void *key,
                                    size_t key_sz, void *value, size_t value_sz,
                                    __u64 flags) {
    int err;
...
int bpf_map__get_next_key(const struct bpf_map *map, const void *cur_key,
...
static int bpf_link_perf_detach(struct bpf_link *link) {
    struct bpf_link_perf *perf_link =
        container_of(link, struct bpf_link_perf, link);
    int err = 0;
    ...
    if (kernel_supports(prog->obj, FEAT_PERF_LINK)) {
        DECLARE_LIBBPF_OPTS(bpf_link_create_opts, link_opts,
                            .perf_event.bpf_cookie =
                                OPTS_GET(opts, bpf_cookie, 0));

        link_fd = bpf_link_create(prog_fd, pfd, BPF_PERF_EVENT, &link_opts);
        if (link_fd < 0) {
            err = -errno;
            pr_warn("prog '%s': failed to create BPF link for perf_event FD "
                    "%d: %d (%s)\n",
                    prog->name, pfd, err,
        ...
        if (ioctl(pfd, PERF_EVENT_IOC_SET_BPF, prog_fd) < 0) {
            ...
            pr_warn("prog '%s': failed to attach to perf_event FD %d: %s\n",
                    ...
                    libbpf_strerror_r(err, errmsg, sizeof(errmsg)));
            if (err == -EPROTO)
                pr_warn("prog '%s': try add PERF_SAMPLE_CALLCHAIN to or remove "
...
static int perf_event_kprobe_open_legacy(const char *probe_name, bool retprobe,
    ...
    if (type < 0) {
        err = type;
        pr_warn("failed to determine legacy kprobe event id for '%s+0x%zx': "
                "%s\n",
                kfunc_name, offset,
                libbpf_strerror_r(err, errmsg, sizeof(errmsg)));
...
bpf_program__attach_kprobe_opts(const struct bpf_program *prog,
    ...
    if (!legacy) {
        ...
        gen_kprobe_legacy_event_name(probe_name, sizeof(probe_name), func_name,
                                     offset);
...
static int resolve_kprobe_multi_cb(unsigned long long sym_addr, char sym_type,
                                   const char *sym_name, void *ctx) {
...
static int attach_ksyscall(const struct bpf_program *prog, long cookie,
    ...
    *link = NULL;
    ...
    if (strcmp(prog->sec_name, "ksyscall") == 0
        || strcmp(prog->sec_name, "kretsyscall") == 0)
        return 0;
...
static long elf_find_func_offset(const char *binary_path, const char *name) {
    ...
    /* Search SHT_DYNSYM, SHT_SYMTAB for symbol.  This search order is used
     * because if a binary is stripped, it may only have SHT_DYNSYM, and a
     * fully-statically linked binary may not have SHT_DYMSYM, so absence of a
     * section should not be reported as a warning/error.
     */
    for (i = 0; i < ARRAY_SIZE(sh_types); i++) {
        ...
        if (!symbols) {
            pr_warn("elf: failed to get symbols for symtab section in '%s': "
                    "%s\n",
        ...
        for (idx = 0; idx < nr_syms; idx++) {
            ...
            if (strncmp(sname, name, name_len) != 0)
                continue;
            /* ...but we don't want a search for "foo" to match 'foo2" also, so
             * any additional characters in sname should be of the form "@@LIB".
             */
            if (!is_name_qualified && sname[name_len] != '\0'
                && sname[name_len] != '@')
                continue;
...
bpf_program__attach_uprobe_opts(const struct bpf_program *prog, pid_t pid,
    ...
    if (!legacy) {
        ...
        pfd = perf_event_uprobe_open_legacy(legacy_probe, retprobe, binary_path,
                                            func_offset, pid);
    }
    if (pfd < 0) {
        err = -errno;
        pr_warn("prog '%s': failed to create %s '%s:0x%zx' perf event: %s\n",
                ...
                func_offset, libbpf_strerror_r(err, errmsg, sizeof(errmsg)));
        goto err_out;
    }
 ...
 * to a full binary path via bpf_program__attach_uprobe_opts.
 *
 * Specifying uprobe+ ensures we carry out strict matching; either "uprobe" must
 * be specified (and auto-attach is not possible) or the above format is
 * specified for auto-attach.
 */
static int attach_uprobe(const struct bpf_program *prog, long cookie,
                         struct bpf_link **link) {
    ...
    *link = NULL;
    ...
    n = sscanf(prog->sec_name, "%m[^/]/%m[^:]:%m[a-zA-Z0-9_.]+%li", &probe_type,
               &binary_path, &func_name, &offset);
    switch (n) {
    case 1:
        ...
        break;
    case 2:
        pr_warn("prog '%s': section '%s' missing ':function[+offset]' "
                "specification\n",
                prog->name, prog->sec_name);
        break;
    case 3:
    case 4:
        ...
        if (opts.retprobe && offset != 0) {
            pr_warn("prog '%s': uretprobes do not support offset "
                    "specification\n",
                    prog->name);
            break;
        }
...
struct bpf_link *bpf_program__attach_usdt(const struct bpf_program *prog,
    ...
    if (!strchr(binary_path, '/')) {
        err = resolve_full_path(binary_path, resolved_path,
...
static int attach_usdt(const struct bpf_program *prog, long cookie,
    ...
    if (n != 3) {
        pr_warn("invalid section '%s', expected "
...
static int determine_tracepoint_id(const char *tp_category,
    ...
    ret = snprintf(file, sizeof(file), "%s/events/%s/%s/id", tracefs_path(),
                   tp_category, tp_name);
    if (ret < 0)
        return -errno;
    if (ret >= sizeof(file)) {
...
struct bpf_link *bpf_program__attach_tracepoint_opts(
    ...
    if (pfd < 0) {
        pr_warn("prog '%s': failed to create tracepoint '%s/%s' perf event: "
                "%s\n",
...
struct bpf_link *bpf_program__attach_tracepoint(const struct bpf_program *prog,
                                                ...
                                                const char *tp_name) {
    return bpf_program__attach_tracepoint_opts(prog, tp_category, tp_name,
                                               NULL);
}
...
bpf_program__attach_btf_id(const struct bpf_program *prog,
    ...
    pfd = bpf_link_create(prog_fd, 0, bpf_program__expected_attach_type(prog),
                          &link_opts);
    if (pfd < 0) {
        pfd = -errno;
        free(link);
...
bpf_program__attach_iter(const struct bpf_program *prog,
    ...
    link_fd =
        bpf_link_create(prog_fd, target_fd, BPF_TRACE_ITER, &link_create_opts);
    if (link_fd < 0) {
        link_fd = -errno;
        free(link);
...
perf_buffer__open_cpu_buf(struct perf_buffer *pb, struct perf_event_attr *attr,
    ...
    cpu_buf->fd = syscall(__NR_perf_event_open, attr, -1 /* pid */, cpu, -1,
                          PERF_FLAG_FD_CLOEXEC);
    if (cpu_buf->fd < 0) {
        err = -errno;
    ...
    cpu_buf->base = mmap(NULL, pb->mmap_size + pb->page_size,
                         PROT_READ | PROT_WRITE, MAP_SHARED, cpu_buf->fd, 0);
...
static struct perf_buffer *__perf_buffer__new(int map_fd, size_t page_cnt,
    ...
    if (err) {
        ...
        pr_debug("failed to get map info for FD %d; API not supported? "
                 "Ignoring...\n",
                 map_fd);
    } else {
    ...
    for (i = 0, j = 0; i < pb->cpu_cnt; i++) {
        ...
        if (epoll_ctl(pb->epoll_fd, EPOLL_CTL_ADD, cpu_buf->fd, &pb->events[j])
            < 0) {
            err = -errno;
            pr_warn("failed to epoll_ctl cpu #%d perf FD %d: %s\n", cpu,
                    cpu_buf->fd, libbpf_strerror_r(err, msg, sizeof(msg)));
            goto error;
        }
        j++;
    }
...
static int perf_buffer__process_records(struct perf_buffer *pb,
    ...
    ret = perf_event_read_simple(cpu_buf->base, pb->mmap_size, pb->page_size,
                                 &cpu_buf->buf, &cpu_buf->buf_size,
...
int perf_buffer__consume(struct perf_buffer *pb) {
    ...
    for (i = 0; i < pb->cpu_cnt; i++) {
        ...
        err = perf_buffer__process_records(pb, cpu_buf);
        if (err) {
            pr_warn("perf_buffer: failed to process records in buffer #%d: "
                    "%d\n",
                    i, err);
            return libbpf_err(err);
        }
    ...
    for (var_idx = 0; var_idx < s->var_cnt; var_idx++) {
        ...
        if (!btf_is_datasec(map_type)) {
            pr_warn("type for map '%1$s' is not a datasec: %2$s",
                    bpf_map__name(map), __btf_kind_str(btf_kind(map_type)));
            return libbpf_err(-EINVAL);
        }
