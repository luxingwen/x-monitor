...
- 计算**cpu system**的占用率，计算结果是0.2%，公式：increase(node_cpu_jiffies{mode="system", cpu="cpu"}[20s])/16000。
  ...
  用irate函数来计算system cpu占用率：rate(node_cpu_jiffies{mode="system", cpu="cpu"}[1m])/800，rate函数会除以1m时间来平均增量，最后除以800是因为8core，1s=100jiffies。
  
  ![prometheus-system-usage-rate](E:\develope\x-monitor\doc\prometheus-system-usage-rate.jpg)
