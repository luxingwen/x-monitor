...
- metric：node_cpu_sys_usage:rate20s

  expr：sum(irate(node_cpu_jiffies{mode="system", cpu!="cpu"}[20s]))/count(node_cpu_jiffies{mode="system", cpu!="cpu"})/100

  说明：cpu内核态占用率

- metric：node_cpu_usr_usage:rate20s

  expr：sum(irate(node_cpu_jiffies{mode="user", cpu!="cpu"}[20s]))/count(node_cpu_jiffies{mode="user", cpu!="cpu"})/100

  说明：cpu用户态占用率

- metric：node_cpu_iowait_usage:rate20s

  expr：sum(irate(node_cpu_jiffies{mode="iowait", cpu!="cpu"}[20s]))/count(node_cpu_jiffies{mode="iowait", cpu!="cpu"})/100
