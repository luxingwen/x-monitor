...
static prom_gauge_t *__metric_loadavg_1min = NULL,
...
int32_t init_collector_proc_loadavg() {
    if (unlikely(!__metric_loadavg_1min)) {
        __metric_loadavg_1min = prom_collector_registry_must_register_metric(
            prom_gauge_new("node_load1", "System 1m Load Average", 1,
                           (const char *[]){ "loadavg" }));
    }

    if (unlikely(!__metric_loadavg_5min)) {
        __metric_loadavg_5min = prom_collector_registry_must_register_metric(
            prom_gauge_new("node_load5", "System 5m Load Average", 1,
                           (const char *[]){ "loadavg" }));
    }

    if (unlikely(!__metric_loadavg_15min)) {
        __metric_loadavg_15min = prom_collector_registry_must_register_metric(
            prom_gauge_new("node_load15", "System 15m Load Average", 1,
                           (const char *[]){ "loadavg" }));
    }

    if (unlikely(!__metric_active_processes)) {
        __metric_active_processes =
            prom_collector_registry_must_register_metric(
                prom_gauge_new("node_forks_total", "Total number of forks", 1,
                               (const char *[]){ "system" }));
    }
...
int32_t collector_proc_loadavg(int32_t UNUSED(update_every), usec_t UNUSED(dt),
    ...
    if (unlikely(!__cfg_proc_loadavg_filename)) {
        __cfg_proc_loadavg_filename = appconfig_get_member_str(
            config_path, "monitor_file", __def_proc_loadavg_filename);
    }

    if (unlikely(!__pf_loadavg)) {
        __pf_loadavg = procfile_open(__cfg_proc_loadavg_filename, " \t,:|/",
                                     PROCFILE_FLAG_DEFAULT);
        if (unlikely(!__pf_loadavg)) {
            error("[PLUGIN_PROC:proc_loadavg] Cannot open %s",
                  __cfg_proc_loadavg_filename);
            return -1;
        }
        debug("[PLUGIN_PROC:proc_loadavg] opened '%s'",
              __cfg_proc_loadavg_filename);
    }
    ...
    if (unlikely(procfile_lines(__pf_loadavg) < 1)) {
        error("%s: File does not contain enough lines.",
              __cfg_proc_loadavg_filename);
        return -1;
    }

    if (unlikely(procfile_linewords(__pf_loadavg, 0) < 6)) {
        error("%s: File does not contain enough columns.",
              __cfg_proc_loadavg_filename);
        return -1;
    }
    ...
    // /proc/stat中有该数值
    // uint64_t running_processes = str2ull(procfile_lineword(__pf_loadavg, 0,
    // 3));
    uint64_t active_processes = str2ull(procfile_lineword(__pf_loadavg, 0, 4));
    pid_t last_running_pid =
        (pid_t)strtoll(procfile_lineword(__pf_loadavg, 0, 5), NULL, 10);

    debug("[PLUGIN_PROC:proc_loadavg] LOAD AVERAGE: %.2f %.2f %.2f "
          "active_processes: %lu "
          "last_running_pid: %d",
