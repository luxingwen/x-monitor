...
if [[ "$cgroup_version" == "cgroup V1" ]]; then
    ...
    cgexec -g memory,cpu:/user.slice/x-monitor.ebpf nohup ./x-monitor.eBPF --config=../../env/config/xm_ebpf_plugin/config.yaml --log_dir=/var/log/x-monitor/ --logtostderr=false --v=5 >/dev/null 2>&1 &
