...
| SoftLockup              |                                                              | 待实现                                           |
| HungTask                |                                                              | 待实现                                           |
| XFS                     | trace_xfs_alloc_file_space<br />trace_xfs_trans_cancel<br />trace_xfs_trans_commit<br />trace_xfs_log_assign_tail_lsn（xfs_log.c,xlog_assign_tail_lsn_locked,从ail列表中找到最小的xfs_log_item的lsn），表明这个lsn是写入磁盘的，也就是tail_lsn<br />trace_xfs_alloc_vextent_nofix<br />trace_xfs_alloc_vextent_loopfailed<br />trace_xfs_alloc_vextent_allfailed<br />trace_xfs_alloc_small_notenough<br />trace_xfs_alloc_small_error<br />trace_xfs_alloc_exact_error<br />trace_xfs_alloc_size_error：后面会调用xfs_btree_del_cursor<br />trace_xfs_perag_get<br />trace_xfs_log_assign_tail_lsn<br />trace_xfs_agf：跟踪agf内存中元数据的变化，这个对我很有意义<br />trace_xfs_read_agf：读取一个agf，第二个参数是ag号<br />trace_xfs_trans_commit_items：当事务的 xfs_log_item 被加入到 cil 链表后，触发 cil 后台任务 | 待实现                                           |
