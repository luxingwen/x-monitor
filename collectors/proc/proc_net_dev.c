...
static const char *__metric_help_net_dev_rx_bytes = "Number of good received "
...
static const char *__metric_help_net_dev_rx_packets =
    ...
    "counts all good "
    "packets received from the device by the host, including packets which "
    "host had to drop at "
...
static const char *__metric_help_net_dev_rx_errors =
    ...
    "must include events "
    "counted by rx_length_errors, rx_crc_errors, rx_frame_errors and other "
    "errors not otherwise "
    "counted.";
static const char *__metric_help_net_dev_rx_dropped =
    "Number of packets received but not processed, e.g. due to lack of "
    "resources or unsupported "
    "protocol. For hardware interfaces this counter may include packets "
    "discarded due to L2 "
    "address filtering but should not include packets dropped by the device "
    "due to buffer "
    "exhaustion which are counted separately in rx_missed_errors (since procfs "
    "folds those two "
    "counters together).";

// FIFO缓冲区错误的数量
static const char *__metric_help_net_dev_rx_fifo = "The number of receive FIFO "
                                                   "buffer errors.";
static const char *__metric_help_net_dev_rx_frame = "The number of receive "
                                                    "framing errors.";
...
static const char *__metric_help_net_dev_rx_multicast =
    ...
    "commonly calculated at "
    "the device level (unlike rx_packets) and therefore may include packets "
    "which did not reach "
    "the host.";
static const char *__metric_help_net_dev_tx_bytes = "Number of good "
                                                    "transmitted bytes, "
                                                    "corresponding to "
                                                    "tx_packets.";
...
static const char *__metric_help_net_dev_tx_dropped = "Number of packets "
                                                      "dropped on their way to "
                                                      "transmission, e.g. due "
                                                      "to lack of resources.";
static const char *__metric_help_net_dev_tx_fifo = "The number of transmit "
                                                   "FIFO buffer errors.";
static const char *__metric_help_net_dev_tx_collisions = "Number of collisions "
                                                         "during packet "
                                                         "transmissions.";
static const char *__metric_help_net_dev_tx_carrier = "Number of frame "
                                                      "transmission errors due "
                                                      "to loss of carrier "
                                                      "during transmission.";
static const char *__metric_help_net_dev_tx_compressed =
    "Number of transmitted compressed packets. This counters is only "
...
static prom_gauge_t *__metric_node_network_transmit_bytes_total = NULL,
                    ...
                    *__metric_node_network_speed = NULL,
                    *__metric_node_network_up = NULL;
...
static struct net_dev_metric {
    ...
    // The total number of packets dropped by the device
    // driver.（设备驱动程序丢弃的数据包总数）
    uint64_t rx_drop;
    ...
    // The number of compressed packets transmitted or received by the device
    // driver. (This appears to be unused in the 2.2.15
    // kernel.)（设备驱动程序发送或接收的压缩数据包数）
    ...
    // The number of collisions detected on the
    // interface.（接口上检测到的冲突数）
    uint64_t tx_colls;
    ...
    // Indicates the interface currently configured MTU value, in bytes, and in
    // decimal format. Specific values depends on the lower-level interface
    // protocol used. Ethernet devices will show a 'mtu' attribute value of 1500
    // unless changed.
    int64_t mtu;
    int64_t speed;
...
static struct net_dev_metric *__get_net_dev_metric(const char *dev_name,
    ...
    // search from begin
    for (m = __net_dev_metric_root; m != __net_dev_metric_last_used;
         m = m->next) {
        if (hash == m->hash && strcmp(m->dev_name, dev_name) == 0) {
            __net_dev_metric_last_used = m->next;
            return m;
    ...
    debug("[PLUGIN_PROC:proc_net_dev] Adding network device metric: '%s' hash: "
          "%u",
          dev_name, m->hash);
    ...
    const char *net_dev_file_fmt = appconfig_get_member_str(
        config_path, "path_netclass_dev_mtu", "/sys/class/net/%s/mtu");
    snprintf(net_dev_property_file, FILENAME_MAX, net_dev_file_fmt, dev_name);
    read_file_to_int64(net_dev_property_file, &m->mtu);

    prom_gauge_set(__metric_node_network_mtu_bytes, (double)m->mtu,
                   (const char *[]){ dev_name });

    // 判断是否是虚拟网卡
    net_dev_file_fmt = appconfig_get_member_str(
        config_path, "path_dev_is_virtual", "/sys/devices/virtual/net/%s");
    snprintf(net_dev_property_file, FILENAME_MAX, net_dev_file_fmt, dev_name);
    // 判断目录是否存在
    ...
    // 获取speed
    net_dev_file_fmt = appconfig_get_member_str(
        config_path, "path_netclass_dev_speed", "/sys/class/net/%s/speed");
    snprintf(net_dev_property_file, FILENAME_MAX, net_dev_file_fmt, dev_name);
    if (read_file_to_int64(net_dev_property_file, &m->speed) == 0) {
        prom_gauge_set(__metric_node_network_speed, (double)m->speed,
                       (const char *[]){ dev_name });
    }
    ...
    net_dev_file_fmt =
        appconfig_get_member_str(config_path, "path_netclass_dev_operstate",
                                 "/sys/class/net/%s/operstate");
    snprintf(net_dev_property_file, FILENAME_MAX, net_dev_file_fmt, dev_name);
    net_dev_property_content_len =
        read_file(net_dev_property_file, net_dev_property_content,
                  sizeof(net_dev_property_content));
    if (net_dev_property_content_len > 0) {
        if (strncmp(net_dev_property_content, "up", 2) == 0) {
            prom_gauge_set(__metric_node_network_up, 1,
                           (const char *[]){ dev_name });
        } else {
            prom_gauge_set(__metric_node_network_up, 0,
                           (const char *[]){ dev_name });
        }
    ...
    // 读取address
    net_dev_file_fmt = appconfig_get_member_str(
        config_path, "path_netclass_dev_address", "/sys/class/net/%s/address");
    snprintf(net_dev_property_file, FILENAME_MAX, net_dev_file_fmt, dev_name);
    net_dev_property_content_len =
        read_file(net_dev_property_file, net_dev_property_content,
                  sizeof(net_dev_property_content));
    ...
    // 读取broadcast
    net_dev_file_fmt =
        appconfig_get_member_str(config_path, "path_netclass_dev_broadcast",
    ...
    // 读取度duplex
    net_dev_file_fmt = appconfig_get_member_str(
        config_path, "path_netclass_dev_duplex", "/sys/class/net/%s/duplex");
    snprintf(net_dev_property_file, FILENAME_MAX, net_dev_file_fmt, dev_name);
    net_dev_property_content_len =
        read_file(net_dev_property_file, net_dev_property_content,
                  sizeof(net_dev_property_content));
    ...
    // 读取tx_queue_len
    net_dev_file_fmt =
        appconfig_get_member_str(config_path, "path_netclass_dev_tx_queue_len",
                                 "/sys/class/net/%s/tx_queue_len");
    snprintf(net_dev_property_file, FILENAME_MAX, net_dev_file_fmt, dev_name);
    if (read_file_to_int64(net_dev_property_file, &m->tx_queue_len) == 0) {
        prom_gauge_set(__metric_node_network_transmit_queue_length,
                       (double)m->tx_queue_len, (const char *[]){ dev_name });
    }

    debug("[PLUGIN_PROC:proc_net_dev] network interface:'%s' mtu:%ld, "
          "speed:%ld, virtual:%d, "
          "operstate:'%s', address:'%s', broadcast:'%s', duplex:'%s', "
          "tx_queue_len:%ld",
          dev_name, m->mtu, m->speed, m->virtual, m->operstate, m->address,
          m->broadcast, m->duplex, m->tx_queue_len);

    prom_gauge_set(__metric_node_network_info, 1,
                   (const char *[]){ dev_name, m->address, m->broadcast,
                                     m->operstate, m->duplex });
...
static void __free_net_dev_metric(struct net_dev_metric *d) {
    if (likely(d)) {
        debug("[PLUGIN_PROC:proc_net_dev] free network interface metric '%s'",
              d->dev_name);
        ...
        // *)d->metric_duplex)->dev_name);
        // prom_map_delete(default_collector->metrics,
...
static void __cleanup_net_dev_metric() {
    ...
    while (d) {
        if (unlikely(!d->updated)) {
            // 如果这个设备没有被检测到
            debug("[PLUGIN_PROC:proc_net_dev] Removing network device metric: "
                  "'%s', linked after "
                  "'%s'",
...
int32_t init_collector_proc_net_dev() {
    // 构造指标并注册
    __metric_node_network_receive_bytes_total =
        prom_collector_registry_must_register_metric(prom_gauge_new(
            "node_network_receive_bytes_total", __metric_help_net_dev_rx_bytes,
            1, (const char *[]){ "device" }));
    __metric_node_network_receive_packets_total =
        prom_collector_registry_must_register_metric(prom_gauge_new(
            "node_network_receive_packets_total",
            __metric_help_net_dev_rx_packets, 1, (const char *[]){ "device" }));
    __metric_node_network_receive_errs_total =
        prom_collector_registry_must_register_metric(prom_gauge_new(
            "node_network_receive_errs_total", __metric_help_net_dev_rx_errors,
            1, (const char *[]){ "device" }));
    __metric_node_network_receive_drop_total =
        prom_collector_registry_must_register_metric(prom_gauge_new(
            "node_network_receive_drop_total", __metric_help_net_dev_rx_dropped,
            1, (const char *[]){ "device" }));
    __metric_node_network_receive_fifo_total =
        prom_collector_registry_must_register_metric(prom_gauge_new(
            "node_network_receive_fifo_total", __metric_help_net_dev_rx_fifo, 1,
            (const char *[]){ "device" }));
    __metric_node_network_receive_frame_total =
        prom_collector_registry_must_register_metric(prom_gauge_new(
            "node_network_receive_frame_total", __metric_help_net_dev_rx_frame,
            1, (const char *[]){ "device" }));
    __metric_node_network_receive_compressed_total =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_network_receive_compressed_total",
                           __metric_help_net_dev_rx_compressed, 1,
                           (const char *[]){ "device" }));
    __metric_node_network_receive_multicast_total =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_network_receive_multicast_total",
                           __metric_help_net_dev_rx_multicast, 1,
                           (const char *[]){ "device" }));
    __metric_node_network_transmit_bytes_total =
        prom_collector_registry_must_register_metric(prom_gauge_new(
            "node_network_transmit_bytes_total", __metric_help_net_dev_tx_bytes,
            1, (const char *[]){ "device" }));
    __metric_node_network_transmit_packets_total =
        prom_collector_registry_must_register_metric(prom_gauge_new(
            "node_network_transmit_packets_total",
            __metric_help_net_dev_tx_packets, 1, (const char *[]){ "device" }));
    __metric_node_network_transmit_errs_total =
        prom_collector_registry_must_register_metric(prom_gauge_new(
            "node_network_transmit_errs_total", __metric_help_net_dev_tx_errors,
            1, (const char *[]){ "device" }));
    __metric_node_network_transmit_drop_total =
        prom_collector_registry_must_register_metric(prom_gauge_new(
            "node_network_transmit_drop_total",
            __metric_help_net_dev_tx_dropped, 1, (const char *[]){ "device" }));
    __metric_node_network_transmit_fifo_total =
        prom_collector_registry_must_register_metric(prom_gauge_new(
            "node_network_transmit_fifo_total", __metric_help_net_dev_tx_fifo,
            1, (const char *[]){ "device" }));
    __metric_node_network_transmit_colls_total =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_network_transmit_colls_total",
                           __metric_help_net_dev_tx_collisions, 1,
                           (const char *[]){ "device" }));
    __metric_node_network_transmit_carrier_total =
        prom_collector_registry_must_register_metric(prom_gauge_new(
            "node_network_transmit_carrier_total",
            __metric_help_net_dev_tx_carrier, 1, (const char *[]){ "device" }));
    __metric_node_network_transmit_compressed_total =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_network_transmit_compressed_total",
                           __metric_help_net_dev_tx_compressed, 1,
                           (const char *[]){ "device" }));

    // ---------------------网络设备的属性---------------------
    __metric_node_network_mtu_bytes =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_network_mtu_bytes",
                           "Indicates the interface currently configured MTU "
                           "value, in bytes, and in decimal format.",
                           1, (const char *[]){ "device" }));
    __metric_node_network_virtual_device =
        prom_collector_registry_must_register_metric(
            prom_counter_new("node_network_virtual_device",
                             "Indicates whether the interface is a virtual "
                             "device.",
                             1, (const char *[]){ "device" }));

    __metric_node_network_info = prom_collector_registry_must_register_metric(
        prom_gauge_new("node_network_info",
                       "Non-numeric data from /sys/class/net/<iface>, value is "
                       "always 1.",
                       5,
                       (const char *[]){ "device", "address", "broadcast",
                                         "operstate", "duplex" }));

    __metric_node_network_transmit_queue_length =
        prom_collector_registry_must_register_metric(
            prom_gauge_new("node_network_transmit_queue_length",
                           "Indicates the interface transmit queue len in "
                           "number of packets, as an "
                           "integer value. Value depend on the type of "
                           "interface, Ethernet network "
                           "adapters have a default value of 1000 unless "
                           "configured otherwise",
                           1, (const char *[]){ "device" }));

    __metric_node_network_speed = prom_collector_registry_must_register_metric(
        prom_gauge_new("node_network_speed",
                       "speed value of /sys/class/net/<iface>.", 1,
                       (const char *[]){ "device" }));

    __metric_node_network_up = prom_collector_registry_must_register_metric(
        prom_gauge_new("node_network_up",
                       "Value is 1 if operstate is 'up', 0 otherwise.", 1,
                       (const char *[]){ "device" }));
...
int32_t collector_proc_net_dev(int32_t UNUSED(update_every), usec_t UNUSED(dt),
    ...
    if (!unlikely(__cfg_proc_net_dev_filename)) {
        __cfg_proc_net_dev_filename = appconfig_get_member_str(
            config_path, "monitor_file", __def_proc_net_dev_filename);
    }

    if (unlikely(!__pf_net_dev)) {
        __pf_net_dev = procfile_open(__cfg_proc_net_dev_filename, " \t,|",
                                     PROCFILE_FLAG_DEFAULT);
        if (unlikely(!__pf_net_dev)) {
            error("[PLUGIN_PROC:proc_net_dev] Cannot open %s",
                  __cfg_proc_net_dev_filename);
            return -1;
        }
        debug("[PLUGIN_PROC:proc_net_dev] opened '%s'",
              __cfg_proc_net_dev_filename);
    }
    ...
    for (size_t l = 2; l < lines; l++) {
        ...
        prom_gauge_set(__metric_node_network_receive_bytes_total, d->rx_bytes,
                       (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_network_receive_packets_total,
                       d->rx_packets, (const char *[]){ dev_name });
        ...
        prom_gauge_set(__metric_node_network_receive_frame_total, d->rx_frame,
                       (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_network_receive_compressed_total,
                       d->rx_compressed, (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_network_receive_multicast_total,
                       d->rx_multicast, (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_network_transmit_bytes_total, d->tx_bytes,
                       (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_network_transmit_packets_total,
                       d->tx_packets, (const char *[]){ dev_name });
        ...
        prom_gauge_set(__metric_node_network_transmit_colls_total, d->tx_colls,
                       (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_network_transmit_carrier_total,
                       d->tx_carrier, (const char *[]){ dev_name });
        prom_gauge_set(__metric_node_network_transmit_compressed_total,
                       d->tx_compressed, (const char *[]){ dev_name });

        debug("[PLUGIN_PROC:proc_net_dev] '%s' rx_bytes: %lu, rx_packets: %lu, "
              "rx_errs: %lu, "
              "rx_drop: %lu, rx_fifo: %lu, rx_frame: %lu, rx_compressed: %lu, "
              "rx_multicast: %lu, "
              "tx_bytes: %lu, tx_packets: %lu, tx_errs: %lu, tx_drop: %lu, "
              "tx_fifo: %lu, tx_colls: "
              "%lu, tx_carrier: %lu, tx_compressed: %lu",
              dev_name, d->rx_bytes, d->rx_packets, d->rx_errs, d->rx_drop,
              d->rx_fifo, d->rx_frame, d->rx_compressed, d->rx_multicast,
              d->tx_bytes, d->tx_packets, d->tx_errs, d->tx_drop, d->tx_fifo,
              d->tx_colls, d->tx_carrier, d->tx_compressed);
    }
