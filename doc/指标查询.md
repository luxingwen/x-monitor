...
- metric：node_load1、node_load15、node_load5
...
- metric：node_cpu_sys_usage
  ...
  expr：sum(rate(node_cpu_jiffies{mode="system", cpu="cpu"}[40s])) by (instance, mode)/count(node_cpu_jiffies{mode="system", cpu!="cpu"}) by (instance, mode)/100

  说明：cpu内核态占用率

- metric：node_cpu_usr_usage

  expr：sum(rate(node_cpu_jiffies{mode="user", cpu="cpu"}[40s])) by (instance, mode)/count(node_cpu_jiffies{mode="user", cpu!="cpu"}) by (instance, mode)/100

  说明：cpu用户态占用率

- metric：node_cpu_iowait_usage

  expr：sum(rate(node_cpu_jiffies{mode="iowait", cpu="cpu"}[40s])) by (instance, mode)/count(node_cpu_jiffies{mode="iowait", cpu!="cpu"}) by (instance, mode)/100

  说明：

- metric：node_cpu_idle_usage

  expr：sum(rate(node_cpu_jiffies{mode="idle", cpu="cpu"}[40s])) by (instance, mode)/count(node_cpu_jiffies{mode="idle", cpu!="cpu"}) by (instance, mode)/100

  说明：

- metric：node_cpu_irq_usage

  expr：sum(rate(node_cpu_jiffies{mode="irq", cpu="cpu"}[40s])) by (instance, mode)/count(node_cpu_jiffies{mode="irq", cpu!="cpu"}) by (instance, mode)/100

  说明：cpu硬中断占有率

- metric：node_cpu_irq_usage

  expr：sum(rate(node_cpu_jiffies{mode="softirq", cpu="cpu"}[40s])) by (instance, mode)/count(node_cpu_jiffies{mode="softirq", cpu!="cpu"}) by (instance, mode)/100

  说明：cpu软中断占有率

- metric：node_cpu_nice_usage

  expr：sum(rate(node_cpu_jiffies{mode="nice", cpu="cpu"}[40s])) by (instance, mode)/count(node_cpu_jiffies{mode="nice", cpu!="cpu"}) by (instance, mode)/100
