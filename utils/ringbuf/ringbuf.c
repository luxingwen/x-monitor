 ...
 */
void ringbuf_get_sizes(unsigned nworkers, size_t *ringbuf_size,
...
ssize_t ringbuf_acquire(ringbuf_t *rbuf, ringbuf_worker_t *w, size_t len) {
    ...
    do {
        ...
        if (__predict_false(next < written && target >= written)) {
            ...
            atomic_store_explicit(&w->seen_off, RBUF_OFF_MAX,
    ...
    if (__predict_false(target & WRAP_LOCK_BIT)) {
         ...
         */
        atomic_store_explicit(&rbuf->next, (target & ~WRAP_LOCK_BIT),
                              memory_order_release);
    }
...
retry:
    ...
    if (next < written) {
        ...
        if (ready == RBUF_OFF_MAX && written == end) {
             ...
             */
            written = 0;
            atomic_store_explicit(&rbuf->written, written,
                                  memory_order_release);
            goto retry;
        }
