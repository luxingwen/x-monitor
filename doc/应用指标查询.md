...
- **app_cpu_usage**
  ...
  expr：sum(rate(app_cpu_jiffies{app="mysqld", instance="127.0.0.1:31079"}[40s])) by (app, instance) / scalar(count(node_cpu_jiffies{mode="iowait", cpu!="cpu"}))

  说明：应用的 cpu 占用率，单位百分比。应用每秒的 jiffies 除以核数、除以 100。**不同的 app 标签的值不同，例如 app="pgsql"**。

- **app_cpu_sys_usage**

  expr：sum(rate(app_stime{app="mysqld", instance="127.0.0.1:31079"}[40s]) + rate(app_cstime{app="mysqld", instance="127.0.0.1:31079"}[40s])) by (app, instance) / scalar(count(node_cpu_jiffies{mode="*iowait*", cpu!="cpu"}))
...
- **app_major_page_faults_per_sec**
  ...
  expr：irate(dbapp_major_page_faults_total{app="*mysqld*"}[40s])，不同的app大括号中标签值不同。
