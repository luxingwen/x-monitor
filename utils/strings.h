...
extern int32_t            strsplit_to_nums(const char *str, const char *delim, uint64_t *nums,
...
static __always_inline void strupr(char *str) {
    if (likely(NULL != str)) {
        char *s = str;
        while (*s) {
            *s = (*s >= 'a' && *s <= 'z') ? *s - 32 : *s;
...
static __always_inline void strlwr(char *str) {
    if (likely(NULL != str)) {
        char *s = str;
        while (*s) {
            *s = (*s >= 'A' && *s <= 'Z') ? *s + 32 : *s;
