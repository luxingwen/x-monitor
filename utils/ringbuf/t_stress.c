 ...
 */
static size_t generate_message(unsigned char *buf, size_t buflen) {
 ...
 */
static ssize_t verify_message(const unsigned char *buf) {
...
static void *ringbuf_stress(void *arg) {
...
static void run_test(void *func(void *)) {
    ...
    for (unsigned i = 0; i < nworkers; i++) {
        if ((errno = pthread_create(&thr[i], NULL, func, (void *)(uintptr_t)i))
            != 0) {
