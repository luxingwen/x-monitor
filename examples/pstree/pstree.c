 ...
 */
static char *WhatString[] = { "@(#)pstree $Revision: 2.33 $ by Fred Hucht (C) 1993-2007",
                              "@(#)EMail: fred AT thp.Uni-Duisburg.de",
                              "$Id: pstree.c,v 2.33 2009-11-10 22:12:39+01 fred Exp $" };
...
int GetProcessesDirect(void) {
    ...
    do {
        ...
        /* Get process table */
        idx = 0;
        nproc = IFNEW(getprocs(proc, sizeof(struct procsinfo), NULL, 0, &idx, maxnproc),
                      getproc(proc, maxnproc, sizeof(struct procinfo)));
#ifdef DEBUG
        idx = errno; /* Don't ask... */
        if (debug)
            fprintf(stderr, "nproc = %d maxnproc = %d" IFNEW(" idx = %d ", "") "\n", nproc,
                    maxnproc, idx);
        errno = idx;
#endif
#ifdef USE_GETPROCS
    ...
    for (i = 0; i < nproc; i++) {
#ifndef USE_GETPROCS
        getuser(&proc[i], sizeof(struct procinfo), &user, sizeof(struct userinfo));
#endif
        ...
        if (debug)
            fprintf(stderr,
                    ...
                    IFNEW(proc[i].pi_tsize, user.ui_tsize), IFNEW(proc[i].pi_dvm, user.ui_dvm),
                    proc[i].pi_thcount, strlen(P[i].cmd), P[i].cmd);
#endif
...
int GetProcessesDirect(void) {
    ...
    for (i = j = 0; i < globbuf.gl_pathc; i++) {
        ...
        fscanf(tn, "%ld %s %*c %ld %ld", &P[j].pid, P[j].cmd, &P[j].ppid, &P[j].pgid);
        fstat(fileno(tn), &stat);
        P[j].uid = stat.st_uid;
        fclose(tn);
        P[j].thcount = 1;

        snprintf(name, sizeof(name), "%s%s", globbuf.gl_pathv[globbuf.gl_pathc - i - 1],
                 "/cmdline");
        tn = fopen(name, "r");
        if (tn == NULL)
            continue;
...
#ifdef DEBUG
            if (debug)
                fprintf(stderr, "dropped line, num=%d != %d\n", num, PSVARSN);
#endif
            continue;
        }
...
#ifdef DEBUG
                if (debug)
                    fprintf(stderr, "fixed zombie %s with ppid %d\n", P[me].cmd, P[me].ppid);
#endif
            }
        }
}
    ...
    for (me = 0; me < NProc; me++) {
        if (showall) {
            ...
            if (0 == strcmp(P[me].name, name)                                        /* for -u */
                ...
                || P[me].pid == ipid                                                 /* for -p */
                || (soption && NULL != strstr(P[me].cmd, str) && P[me].pid != MyPid) /* for -s */
            ) {
                P[me].selected = TRUE;
                /* Mark parents */
...
void DropProcs(void) {
    ...
    for (me = 0; me < NProc; me++)
        if (P[me].print) {
            ...
            for (child = P[me].child; EXIST(child) && !P[child].print; child = P[child].sister)
                ;
            P[me].child = child;
...
void PrintTree(int idx, const char *head) {
    ...
    snprintf(out, sizeof(out), "%s%s%s%s%s%s %5ld %s %s%s" /*" (ch=%d, si=%d, pr=%d)"*/, C->sg,
             head,
             head[0] == '\0'      ? "" :
             EXIST(P[idx].sister) ? C->barc :
                                    C->barl,
             EXIST(P[idx].child) ? C->p : C->s2, P[idx].pid == P[idx].pgid ? C->pgl : C->npgl,
             C->eg, P[idx].pid, !noption ? P[idx].name : "", thread, P[idx].cmd
...
#ifdef DEBUG
    if (debug)
        fprintf(stderr, "NProc = %d processes found.\n", NProc);
#endif
